#include "simple-decentralized-lock.node_1.finite.h"
...
#include <cstdint>
#endif
typedef simple_decentralized_lock__node_1__finite ivy_class;
...
void simple_decentralized_lock__node_1__finite::install_reader(reader *r) {
    #ifdef _WIN32
...
void simple_decentralized_lock__node_1__finite::install_thread(reader *r) {
    install_reader(r);
}

void simple_decentralized_lock__node_1__finite::install_timer(timer *r) {
    #ifdef _WIN32
...
#ifdef _WIN32
    void simple_decentralized_lock__node_1__finite::__lock() { WaitForSingleObject(mutex,INFINITE); }
    void simple_decentralized_lock__node_1__finite::__unlock() { ReleaseMutex(mutex); }
#else
    void simple_decentralized_lock__node_1__finite::__lock() { pthread_mutex_lock(&mutex); }
    void simple_decentralized_lock__node_1__finite::__unlock() { pthread_mutex_unlock(&mutex); }
#endif
...
std::ostream &operator <<(std::ostream &s, const simple_decentralized_lock__node_1__finite::node &t);
template <>
simple_decentralized_lock__node_1__finite::node _arg<simple_decentralized_lock__node_1__finite::node>(std::vector<ivy_value> &args, unsigned idx, long long bound);
template <>
void  __ser<simple_decentralized_lock__node_1__finite::node>(ivy_ser &res, const simple_decentralized_lock__node_1__finite::node&);
template <>
void  __deser<simple_decentralized_lock__node_1__finite::node>(ivy_deser &inp, simple_decentralized_lock__node_1__finite::node &res);
int simple_decentralized_lock__node_1__finite::___ivy_choose(int rng,const char *name,int id) {
        return 0;
    }
struct ivy_nondet_except {}; // lauren-yrluo added
void simple_decentralized_lock__node_1__finite::__init(){
        bool __tmp0[1][1];
...
void simple_decentralized_lock__node_1__finite::ext__send(node src, node dst){
        ivy_assume(has_lock[src], "simple-decentralized-lock.node_1.finite.ivy: line 18");
        message[src][dst] = true;
        has_lock[src] = false;
}
void simple_decentralized_lock__node_1__finite::ext__recv(node src, node dst){
        ivy_assume(message[src][dst], "simple-decentralized-lock.node_1.finite.ivy: line 24");
        message[src][dst] = false;
        has_lock[dst] = true;
}
simple_decentralized_lock__node_1__finite::node simple_decentralized_lock__node_1__finite::ext__get_start_node(){
    simple_decentralized_lock__node_1__finite::node qrm_result;
    qrm_result = start_node;
    return qrm_result;
}
bool simple_decentralized_lock__node_1__finite::ext__get_bool_start_node(node result){
    bool qrm_result;
    qrm_result = (start_node == result);
    return qrm_result;
}
bool simple_decentralized_lock__node_1__finite::ext__get_has_lock(node n0){
    bool qrm_result;
    qrm_result = has_lock[n0];
    return qrm_result;
}
bool simple_decentralized_lock__node_1__finite::ext__get_bool_has_lock(node n0, bool result){
    bool qrm_result;
    qrm_result = (has_lock[n0] == result);
    return qrm_result;
}
bool simple_decentralized_lock__node_1__finite::ext__get_message(node n0, node n1){
    bool qrm_result;
    qrm_result = message[n0][n1];
    return qrm_result;
}
bool simple_decentralized_lock__node_1__finite::ext__get_bool_message(node n0, node n1, bool result){
    bool qrm_result;
    qrm_result = (message[n0][n1] == result);
    return qrm_result;
}
void simple_decentralized_lock__node_1__finite::__tick(int __timeout){
}
simple_decentralized_lock__node_1__finite::simple_decentralized_lock__node_1__finite(){
#ifdef _WIN32
...
for (node X__0 = (node)0; (int) X__0 < 1; X__0 = (node)(((int)X__0) + 1)) {
    for (node X__1 = (node)0; (int) X__1 < 1; X__1 = (node)(((int)X__1) + 1)) {
...
std::ostream &operator <<(std::ostream &s, const simple_decentralized_lock__node_1__finite::node &t){
    if (t == simple_decentralized_lock__node_1__finite::node0) s<<"node0";
    return s;
}
template <>
void  __ser<simple_decentralized_lock__node_1__finite::node>(ivy_ser &res, const simple_decentralized_lock__node_1__finite::node&t){
    __ser(res,(int)t);
}
...
int ask_ret(long long bound) {
    ...
    class simple_decentralized_lock__node_1__finite_repl : public simple_decentralized_lock__node_1__finite {
    ...
    virtual void ivy_assume(bool truth,const char *msg){
        ...
        simple_decentralized_lock__node_1__finite_repl() : simple_decentralized_lock__node_1__finite(){}
...
simple_decentralized_lock__node_1__finite::node _arg<simple_decentralized_lock__node_1__finite::node>(std::vector<ivy_value> &args, unsigned idx, long long bound){
    ...
    if(arg.atom == "node0") return simple_decentralized_lock__node_1__finite::node0;
...
void __deser<simple_decentralized_lock__node_1__finite::node>(ivy_deser &inp, simple_decentralized_lock__node_1__finite::node &res){
    int __res;
    __deser(inp,__res);
    res = (simple_decentralized_lock__node_1__finite::node)__res;
}
...
class cmd_reader: public stdin_reader {
    int lineno;
public:
    simple_decentralized_lock__node_1__finite_repl &ivy;    

    cmd_reader(simple_decentralized_lock__node_1__finite_repl &_ivy) : ivy(_ivy) {
        lineno = 1;
        if (isatty(fdes()))
        ...
        try {
            ...
            ivy.__lock();
                ...
                if (action == "get_bool_has_lock") {
                    ...
                    __ivy_out  << "= " << ivy.ext__get_bool_has_lock(_arg<simple_decentralized_lock__node_1__finite::node>(args,0,1),_arg<bool>(args,1,2)) << std::endl;
                }
                else
    
                if (action == "get_bool_message") {
                    check_arity(args,3,action);
                    __ivy_out  << "= " << ivy.ext__get_bool_message(_arg<simple_decentralized_lock__node_1__finite::node>(args,0,1),_arg<simple_decentralized_lock__node_1__finite::node>(args,1,1),_arg<bool>(args,2,2)) << std::endl;
                }
                else
    
                if (action == "get_bool_start_node") {
                    check_arity(args,1,action);
                    __ivy_out  << "= " << ivy.ext__get_bool_start_node(_arg<simple_decentralized_lock__node_1__finite::node>(args,0,1)) << std::endl;
                }
                else
    
                if (action == "get_has_lock") {
                    check_arity(args,1,action);
                    __ivy_out  << "= " << ivy.ext__get_has_lock(_arg<simple_decentralized_lock__node_1__finite::node>(args,0,1)) << std::endl;
                }
                else
    
                if (action == "get_message") {
                    check_arity(args,2,action);
                    __ivy_out  << "= " << ivy.ext__get_message(_arg<simple_decentralized_lock__node_1__finite::node>(args,0,1),_arg<simple_decentralized_lock__node_1__finite::node>(args,1,1)) << std::endl;
                }
                else
                ...
                if (action == "recv") {
                    check_arity(args,2,action);
                    ivy.ext__recv(_arg<simple_decentralized_lock__node_1__finite::node>(args,0,1),_arg<simple_decentralized_lock__node_1__finite::node>(args,1,1));
                }
                else
    
                if (action == "send") {
                    check_arity(args,2,action);
                    ivy.ext__send(_arg<simple_decentralized_lock__node_1__finite::node>(args,0,1),_arg<simple_decentralized_lock__node_1__finite::node>(args,1,1));
                }
                else
...
int main(int argc, char **argv){
    ...
    if (argc != 1){
        std::cerr << "usage: simple_decentralized_lock__node_1__finite \n";
        __ivy_exit(1);
    }
...
#include <vector>
simple_decentralized_lock__node_1__finite_repl * ivy_exec;
cmd_reader* ivy_exec_cr;
std::ostringstream ivy_exec_stream;

void ivy_exec_init(){
	__ivy_out.basic_ios<char>::rdbuf(ivy_exec_stream.rdbuf());
	ivy_exec = new simple_decentralized_lock__node_1__finite_repl;
	ivy_exec -> __unlock();
...
void ivy_exec_set_state(std::vector<std::string> state_values){
	...
	std::stringstream(state_values[1]) >> ivy_exec -> message[0][0];
	{
		ivy_value arg; arg.pos = 0; arg.atom = state_values[2];
		std::vector<ivy_value> args; args.push_back(arg);
		ivy_exec -> start_node = _arg<simple_decentralized_lock__node_1__finite::node>(args, 0, 0);
	}
}
