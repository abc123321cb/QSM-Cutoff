#include "firewall.node_2.finite.h"
...
#include <cstdint>
#endif
typedef firewall__node_2__finite ivy_class;
...
void firewall__node_2__finite::install_reader(reader *r) {
    #ifdef _WIN32
...
void firewall__node_2__finite::install_thread(reader *r) {
    install_reader(r);
}

void firewall__node_2__finite::install_timer(timer *r) {
    #ifdef _WIN32
...
#ifdef _WIN32
    void firewall__node_2__finite::__lock() { WaitForSingleObject(mutex,INFINITE); }
    void firewall__node_2__finite::__unlock() { ReleaseMutex(mutex); }
#else
    void firewall__node_2__finite::__lock() { pthread_mutex_lock(&mutex); }
    void firewall__node_2__finite::__unlock() { pthread_mutex_unlock(&mutex); }
#endif
...
std::ostream &operator <<(std::ostream &s, const firewall__node_2__finite::node &t);
template <>
firewall__node_2__finite::node _arg<firewall__node_2__finite::node>(std::vector<ivy_value> &args, unsigned idx, long long bound);
template <>
void  __ser<firewall__node_2__finite::node>(ivy_ser &res, const firewall__node_2__finite::node&);
template <>
void  __deser<firewall__node_2__finite::node>(ivy_deser &inp, firewall__node_2__finite::node &res);
int firewall__node_2__finite::___ivy_choose(int rng,const char *name,int id) {
        return 0;
    }
struct ivy_nondet_except {}; // lauren-yrluo added
void firewall__node_2__finite::__init(){
...
void firewall__node_2__finite::ext__send_from_internal(node src, node dst){
        ivy_assume(internal[src], "firewall.node_2.finite.ivy: line 16");
        ivy_assume(!internal[dst], "firewall.node_2.finite.ivy: line 17");
        sent[src][dst] = true;
        allowed_in[dst] = true;
}
void firewall__node_2__finite::ext__send_to_internal(node src, node dst){
        ivy_assume(!internal[src], "firewall.node_2.finite.ivy: line 24");
        ivy_assume(internal[dst], "firewall.node_2.finite.ivy: line 25");
        ivy_assume(allowed_in[src], "firewall.node_2.finite.ivy: line 26");
        sent[src][dst] = true;
}
bool firewall__node_2__finite::ext__get_internal(node n0){
    bool qrm_result;
    qrm_result = internal[n0];
    return qrm_result;
}
bool firewall__node_2__finite::ext__get_bool_internal(node n0, bool result){
    bool qrm_result;
    qrm_result = (internal[n0] == result);
    return qrm_result;
}
bool firewall__node_2__finite::ext__get_sent(node n0, node n1){
    bool qrm_result;
    qrm_result = sent[n0][n1];
    return qrm_result;
}
bool firewall__node_2__finite::ext__get_bool_sent(node n0, node n1, bool result){
    bool qrm_result;
    qrm_result = (sent[n0][n1] == result);
    return qrm_result;
}
bool firewall__node_2__finite::ext__get_allowed_in(node n0){
    bool qrm_result;
    qrm_result = allowed_in[n0];
    return qrm_result;
}
bool firewall__node_2__finite::ext__get_bool_allowed_in(node n0, bool result){
    bool qrm_result;
    qrm_result = (allowed_in[n0] == result);
    return qrm_result;
}
void firewall__node_2__finite::__tick(int __timeout){
}
firewall__node_2__finite::firewall__node_2__finite(){
#ifdef _WIN32
...
std::ostream &operator <<(std::ostream &s, const firewall__node_2__finite::node &t){
    if (t == firewall__node_2__finite::node0) s<<"node0";
    if (t == firewall__node_2__finite::node1) s<<"node1";
    return s;
}
template <>
void  __ser<firewall__node_2__finite::node>(ivy_ser &res, const firewall__node_2__finite::node&t){
    __ser(res,(int)t);
}
...
int ask_ret(long long bound) {
    ...
    class firewall__node_2__finite_repl : public firewall__node_2__finite {
    ...
    virtual void ivy_assume(bool truth,const char *msg){
        ...
        firewall__node_2__finite_repl() : firewall__node_2__finite(){}
...
firewall__node_2__finite::node _arg<firewall__node_2__finite::node>(std::vector<ivy_value> &args, unsigned idx, long long bound){
    ...
    if(arg.atom == "node0") return firewall__node_2__finite::node0;
    if(arg.atom == "node1") return firewall__node_2__finite::node1;
...
void __deser<firewall__node_2__finite::node>(ivy_deser &inp, firewall__node_2__finite::node &res){
    int __res;
    __deser(inp,__res);
    res = (firewall__node_2__finite::node)__res;
}
...
class cmd_reader: public stdin_reader {
    int lineno;
public:
    firewall__node_2__finite_repl &ivy;    

    cmd_reader(firewall__node_2__finite_repl &_ivy) : ivy(_ivy) {
        lineno = 1;
        if (isatty(fdes()))
        ...
        try {
            ...
            ivy.__lock();
                ...
                if (action == "get_allowed_in") {
                    ...
                    __ivy_out  << "= " << ivy.ext__get_allowed_in(_arg<firewall__node_2__finite::node>(args,0,2)) << std::endl;
                }
                else
    
                if (action == "get_bool_allowed_in") {
                    check_arity(args,2,action);
                    __ivy_out  << "= " << ivy.ext__get_bool_allowed_in(_arg<firewall__node_2__finite::node>(args,0,2),_arg<bool>(args,1,2)) << std::endl;
                }
                else
    
                if (action == "get_bool_internal") {
                    check_arity(args,2,action);
                    __ivy_out  << "= " << ivy.ext__get_bool_internal(_arg<firewall__node_2__finite::node>(args,0,2),_arg<bool>(args,1,2)) << std::endl;
                }
                else
    
                if (action == "get_bool_sent") {
                    check_arity(args,3,action);
                    __ivy_out  << "= " << ivy.ext__get_bool_sent(_arg<firewall__node_2__finite::node>(args,0,2),_arg<firewall__node_2__finite::node>(args,1,2),_arg<bool>(args,2,2)) << std::endl;
                }
                else
    
                if (action == "get_internal") {
                    check_arity(args,1,action);
                    __ivy_out  << "= " << ivy.ext__get_internal(_arg<firewall__node_2__finite::node>(args,0,2)) << std::endl;
                }
                else
    
                if (action == "get_sent") {
                    check_arity(args,2,action);
                    __ivy_out  << "= " << ivy.ext__get_sent(_arg<firewall__node_2__finite::node>(args,0,2),_arg<firewall__node_2__finite::node>(args,1,2)) << std::endl;
                }
                else
    
                if (action == "send_from_internal") {
                    check_arity(args,2,action);
                    ivy.ext__send_from_internal(_arg<firewall__node_2__finite::node>(args,0,2),_arg<firewall__node_2__finite::node>(args,1,2));
                }
                else
    
                if (action == "send_to_internal") {
                    check_arity(args,2,action);
                    ivy.ext__send_to_internal(_arg<firewall__node_2__finite::node>(args,0,2),_arg<firewall__node_2__finite::node>(args,1,2));
                }
                else
...
int main(int argc, char **argv){
    ...
    if (argc != 1){
        std::cerr << "usage: firewall__node_2__finite \n";
        __ivy_exit(1);
    }
...
void ivy_exec_set_state(std::vector<std::string> state_values){
	...
	std::stringstream(state_values[2]) >> ivy_exec -> internal[0];
	std::stringstream(state_values[3]) >> ivy_exec -> internal[1];
	std::stringstream(state_values[4]) >> ivy_exec -> sent[0][0];
	std::stringstream(state_values[5]) >> ivy_exec -> sent[0][1];
	std::stringstream(state_values[6]) >> ivy_exec -> sent[1][0];
	std::stringstream(state_values[7]) >> ivy_exec -> sent[1][1];
}
