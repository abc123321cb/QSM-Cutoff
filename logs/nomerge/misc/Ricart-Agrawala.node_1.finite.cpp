#include "Ricart-Agrawala.node_1.finite.h"
...
#include <cstdint>
#endif
typedef Ricart_Agrawala__node_1__finite ivy_class;
...
void Ricart_Agrawala__node_1__finite::install_reader(reader *r) {
    #ifdef _WIN32
...
void Ricart_Agrawala__node_1__finite::install_thread(reader *r) {
    install_reader(r);
}

void Ricart_Agrawala__node_1__finite::install_timer(timer *r) {
    #ifdef _WIN32
...
#ifdef _WIN32
    void Ricart_Agrawala__node_1__finite::__lock() { WaitForSingleObject(mutex,INFINITE); }
    void Ricart_Agrawala__node_1__finite::__unlock() { ReleaseMutex(mutex); }
#else
    void Ricart_Agrawala__node_1__finite::__lock() { pthread_mutex_lock(&mutex); }
    void Ricart_Agrawala__node_1__finite::__unlock() { pthread_mutex_unlock(&mutex); }
#endif
...
std::ostream &operator <<(std::ostream &s, const Ricart_Agrawala__node_1__finite::node &t);
template <>
Ricart_Agrawala__node_1__finite::node _arg<Ricart_Agrawala__node_1__finite::node>(std::vector<ivy_value> &args, unsigned idx, long long bound);
template <>
void  __ser<Ricart_Agrawala__node_1__finite::node>(ivy_ser &res, const Ricart_Agrawala__node_1__finite::node&);
template <>
void  __deser<Ricart_Agrawala__node_1__finite::node>(ivy_deser &inp, Ricart_Agrawala__node_1__finite::node &res);
int Ricart_Agrawala__node_1__finite::___ivy_choose(int rng,const char *name,int id) {
        return 0;
    }
struct ivy_nondet_except {}; // lauren-yrluo added
void Ricart_Agrawala__node_1__finite::__init(){
        bool __tmp0[1][1];
...
void Ricart_Agrawala__node_1__finite::ext__request(node requester, node responder){
        ivy_assume(!requested[requester][responder], "Ricart-Agrawala.node_1.finite.ivy: line 18");
        ivy_assume(!(requester == responder), "Ricart-Agrawala.node_1.finite.ivy: line 19");
        requested[requester][responder] = true;
}
void Ricart_Agrawala__node_1__finite::ext__reply(node requester, node responder){
        ivy_assume(!replied[requester][responder], "Ricart-Agrawala.node_1.finite.ivy: line 24");
        ivy_assume(!holds[responder], "Ricart-Agrawala.node_1.finite.ivy: line 25");
        ivy_assume(!replied[responder][requester], "Ricart-Agrawala.node_1.finite.ivy: line 26");
        ivy_assume(requested[requester][responder], "Ricart-Agrawala.node_1.finite.ivy: line 27");
        ivy_assume(!(requester == responder), "Ricart-Agrawala.node_1.finite.ivy: line 28");
        requested[requester][responder] = false;
        replied[requester][responder] = true;
}
void Ricart_Agrawala__node_1__finite::ext__enter(node requester){
        int __tmp3;
        __tmp3 = 1;
        for (int N = 0; N < 1; N++) {
            if (!(!!(N == requester) || replied[requester][N])) __tmp3 = 0;
        }
        ivy_assume(__tmp3, "Ricart-Agrawala.node_1.finite.ivy: line 34");
        holds[requester] = true;
}
void Ricart_Agrawala__node_1__finite::ext__leave(node requester){
        ivy_assume(holds[requester], "Ricart-Agrawala.node_1.finite.ivy: line 39");
...
bool Ricart_Agrawala__node_1__finite::ext__get_replied(node n0, node n1){
    bool qrm_result;
    qrm_result = replied[n0][n1];
    return qrm_result;
}
bool Ricart_Agrawala__node_1__finite::ext__get_bool_replied(node n0, node n1, bool result){
    bool qrm_result;
    qrm_result = (replied[n0][n1] == result);
    return qrm_result;
}
bool Ricart_Agrawala__node_1__finite::ext__get_holds(node n0){
    bool qrm_result;
    qrm_result = holds[n0];
    return qrm_result;
}
bool Ricart_Agrawala__node_1__finite::ext__get_bool_holds(node n0, bool result){
    bool qrm_result;
    qrm_result = (holds[n0] == result);
    return qrm_result;
}
bool Ricart_Agrawala__node_1__finite::ext__get_requested(node n0, node n1){
    bool qrm_result;
    qrm_result = requested[n0][n1];
    return qrm_result;
}
bool Ricart_Agrawala__node_1__finite::ext__get_bool_requested(node n0, node n1, bool result){
    bool qrm_result;
    qrm_result = (requested[n0][n1] == result);
    return qrm_result;
}
void Ricart_Agrawala__node_1__finite::__tick(int __timeout){
}
Ricart_Agrawala__node_1__finite::Ricart_Agrawala__node_1__finite(){
#ifdef _WIN32
...
std::ostream &operator <<(std::ostream &s, const Ricart_Agrawala__node_1__finite::node &t){
    if (t == Ricart_Agrawala__node_1__finite::node0) s<<"node0";
    return s;
}
template <>
void  __ser<Ricart_Agrawala__node_1__finite::node>(ivy_ser &res, const Ricart_Agrawala__node_1__finite::node&t){
    __ser(res,(int)t);
}
...
int ask_ret(long long bound) {
    ...
    class Ricart_Agrawala__node_1__finite_repl : public Ricart_Agrawala__node_1__finite {
    ...
    virtual void ivy_assume(bool truth,const char *msg){
        ...
        Ricart_Agrawala__node_1__finite_repl() : Ricart_Agrawala__node_1__finite(){}
...
Ricart_Agrawala__node_1__finite::node _arg<Ricart_Agrawala__node_1__finite::node>(std::vector<ivy_value> &args, unsigned idx, long long bound){
    ...
    if(arg.atom == "node0") return Ricart_Agrawala__node_1__finite::node0;
...
void __deser<Ricart_Agrawala__node_1__finite::node>(ivy_deser &inp, Ricart_Agrawala__node_1__finite::node &res){
    int __res;
    __deser(inp,__res);
    res = (Ricart_Agrawala__node_1__finite::node)__res;
}
...
class cmd_reader: public stdin_reader {
    int lineno;
public:
    Ricart_Agrawala__node_1__finite_repl &ivy;    

    cmd_reader(Ricart_Agrawala__node_1__finite_repl &_ivy) : ivy(_ivy) {
        lineno = 1;
        if (isatty(fdes()))
        ...
        try {
            ...
            ivy.__lock();
                ...
                if (action == "enter") {
                    ...
                    ivy.ext__enter(_arg<Ricart_Agrawala__node_1__finite::node>(args,0,1));
                }
                else
    
                if (action == "get_bool_holds") {
                    check_arity(args,2,action);
                    __ivy_out  << "= " << ivy.ext__get_bool_holds(_arg<Ricart_Agrawala__node_1__finite::node>(args,0,1),_arg<bool>(args,1,2)) << std::endl;
                }
                else
    
                if (action == "get_bool_replied") {
                    check_arity(args,3,action);
                    __ivy_out  << "= " << ivy.ext__get_bool_replied(_arg<Ricart_Agrawala__node_1__finite::node>(args,0,1),_arg<Ricart_Agrawala__node_1__finite::node>(args,1,1),_arg<bool>(args,2,2)) << std::endl;
                }
                else
    
                if (action == "get_bool_requested") {
                    check_arity(args,3,action);
                    __ivy_out  << "= " << ivy.ext__get_bool_requested(_arg<Ricart_Agrawala__node_1__finite::node>(args,0,1),_arg<Ricart_Agrawala__node_1__finite::node>(args,1,1),_arg<bool>(args,2,2)) << std::endl;
                }
                else
    
                if (action == "get_holds") {
                    check_arity(args,1,action);
                    __ivy_out  << "= " << ivy.ext__get_holds(_arg<Ricart_Agrawala__node_1__finite::node>(args,0,1)) << std::endl;
                }
                else
    
                if (action == "get_replied") {
                    check_arity(args,2,action);
                    __ivy_out  << "= " << ivy.ext__get_replied(_arg<Ricart_Agrawala__node_1__finite::node>(args,0,1),_arg<Ricart_Agrawala__node_1__finite::node>(args,1,1)) << std::endl;
                }
                else
    
                if (action == "get_requested") {
                    check_arity(args,2,action);
                    __ivy_out  << "= " << ivy.ext__get_requested(_arg<Ricart_Agrawala__node_1__finite::node>(args,0,1),_arg<Ricart_Agrawala__node_1__finite::node>(args,1,1)) << std::endl;
                }
                else
    
                if (action == "leave") {
                    check_arity(args,1,action);
                    ivy.ext__leave(_arg<Ricart_Agrawala__node_1__finite::node>(args,0,1));
                }
                else
    
                if (action == "reply") {
                    check_arity(args,2,action);
                    ivy.ext__reply(_arg<Ricart_Agrawala__node_1__finite::node>(args,0,1),_arg<Ricart_Agrawala__node_1__finite::node>(args,1,1));
                }
                else
    
                if (action == "request") {
                    check_arity(args,2,action);
                    ivy.ext__request(_arg<Ricart_Agrawala__node_1__finite::node>(args,0,1),_arg<Ricart_Agrawala__node_1__finite::node>(args,1,1));
                }
                else
...
int main(int argc, char **argv){
    ...
    if (argc != 1){
        std::cerr << "usage: Ricart_Agrawala__node_1__finite \n";
        __ivy_exit(1);
    }
...
void ivy_exec_set_state(std::vector<std::string> state_values){
	...
	std::stringstream(state_values[1]) >> ivy_exec -> replied[0][0];
	std::stringstream(state_values[2]) >> ivy_exec -> requested[0][0];
}
