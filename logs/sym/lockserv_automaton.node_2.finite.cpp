#include "lockserv_automaton.node_2.finite.h"
...
#include <cstdint>
#endif
typedef lockserv_automaton__node_2__finite ivy_class;
...
void lockserv_automaton__node_2__finite::install_reader(reader *r) {
    #ifdef _WIN32
...
void lockserv_automaton__node_2__finite::install_thread(reader *r) {
    install_reader(r);
}

void lockserv_automaton__node_2__finite::install_timer(timer *r) {
    #ifdef _WIN32
...
#ifdef _WIN32
    void lockserv_automaton__node_2__finite::__lock() { WaitForSingleObject(mutex,INFINITE); }
    void lockserv_automaton__node_2__finite::__unlock() { ReleaseMutex(mutex); }
#else
    void lockserv_automaton__node_2__finite::__lock() { pthread_mutex_lock(&mutex); }
    void lockserv_automaton__node_2__finite::__unlock() { pthread_mutex_unlock(&mutex); }
#endif
...
std::ostream &operator <<(std::ostream &s, const lockserv_automaton__node_2__finite::node &t);
template <>
lockserv_automaton__node_2__finite::node _arg<lockserv_automaton__node_2__finite::node>(std::vector<ivy_value> &args, unsigned idx, long long bound);
template <>
void  __ser<lockserv_automaton__node_2__finite::node>(ivy_ser &res, const lockserv_automaton__node_2__finite::node&);
template <>
void  __deser<lockserv_automaton__node_2__finite::node>(ivy_deser &inp, lockserv_automaton__node_2__finite::node &res);
int lockserv_automaton__node_2__finite::___ivy_choose(int rng,const char *name,int id) {
        return 0;
    }
struct ivy_nondet_except {}; // lauren-yrluo added
void lockserv_automaton__node_2__finite::__init(){
        bool __tmp0[2];
        for (int N = 0; N < 2; N++) {
            __tmp0[N] = false;
        }
...
void lockserv_automaton__node_2__finite::ext__lock(node n){
    lock_msg[n] = true;
}
void lockserv_automaton__node_2__finite::ext__unlock(node n){
        ivy_assume(holds_lock[n], "lockserv_automaton.node_2.finite.ivy: line 32");
        holds_lock[n] = false;
        unlock_msg[n] = true;
}
void lockserv_automaton__node_2__finite::ext__recv_lock(node sender){
        ivy_assume(lock_msg[sender], "lockserv_automaton.node_2.finite.ivy: line 39");
        ivy_assume(!held, "lockserv_automaton.node_2.finite.ivy: line 40");
        held = true;
        lock_msg[sender] = false;
        grant_msg[sender] = true;
}
void lockserv_automaton__node_2__finite::ext__recv_grant(node n){
        ivy_assume(grant_msg[n], "lockserv_automaton.node_2.finite.ivy: line 48");
        grant_msg[n] = false;
        holds_lock[n] = true;
}
void lockserv_automaton__node_2__finite::ext__recv_unlock(node sender){
        ivy_assume(unlock_msg[sender], "lockserv_automaton.node_2.finite.ivy: line 55");
        unlock_msg[sender] = false;
        held = false;
}
bool lockserv_automaton__node_2__finite::ext__get_grant_msg(node n0){
    bool x;
    x = grant_msg[n0];
    return x;
}
bool lockserv_automaton__node_2__finite::ext__get_bool_grant_msg(node n0, bool result){
    bool x;
    x = (grant_msg[n0] == result);
    return x;
}
bool lockserv_automaton__node_2__finite::ext__get_held(){
    bool x;
    x = held;
    return x;
}
bool lockserv_automaton__node_2__finite::ext__get_bool_held(bool result){
    bool x;
    x = (held == result);
    return x;
}
bool lockserv_automaton__node_2__finite::ext__get_holds_lock(node n0){
    bool x;
    x = holds_lock[n0];
    return x;
}
bool lockserv_automaton__node_2__finite::ext__get_bool_holds_lock(node n0, bool result){
    bool x;
    x = (holds_lock[n0] == result);
    return x;
}
bool lockserv_automaton__node_2__finite::ext__get_lock_msg(node n0){
    bool x;
    x = lock_msg[n0];
    return x;
}
bool lockserv_automaton__node_2__finite::ext__get_bool_lock_msg(node n0, bool result){
    bool x;
    x = (lock_msg[n0] == result);
    return x;
}
bool lockserv_automaton__node_2__finite::ext__get_unlock_msg(node n0){
    bool x;
    x = unlock_msg[n0];
    return x;
}
bool lockserv_automaton__node_2__finite::ext__get_bool_unlock_msg(node n0, bool result){
    bool x;
    x = (unlock_msg[n0] == result);
    return x;
}
void lockserv_automaton__node_2__finite::__tick(int __timeout){
}
lockserv_automaton__node_2__finite::lockserv_automaton__node_2__finite(){
#ifdef _WIN32
...
std::ostream &operator <<(std::ostream &s, const lockserv_automaton__node_2__finite::node &t){
    if (t == lockserv_automaton__node_2__finite::node0) s<<"node0";
    if (t == lockserv_automaton__node_2__finite::node1) s<<"node1";
    return s;
}
template <>
void  __ser<lockserv_automaton__node_2__finite::node>(ivy_ser &res, const lockserv_automaton__node_2__finite::node&t){
    __ser(res,(int)t);
}
...
int ask_ret(long long bound) {
    ...
    class lockserv_automaton__node_2__finite_repl : public lockserv_automaton__node_2__finite {
    ...
    virtual void ivy_assume(bool truth,const char *msg){
        ...
        lockserv_automaton__node_2__finite_repl() : lockserv_automaton__node_2__finite(){}
...
lockserv_automaton__node_2__finite::node _arg<lockserv_automaton__node_2__finite::node>(std::vector<ivy_value> &args, unsigned idx, long long bound){
    ...
    if(arg.atom == "node0") return lockserv_automaton__node_2__finite::node0;
    if(arg.atom == "node1") return lockserv_automaton__node_2__finite::node1;
...
void __deser<lockserv_automaton__node_2__finite::node>(ivy_deser &inp, lockserv_automaton__node_2__finite::node &res){
    int __res;
    __deser(inp,__res);
    res = (lockserv_automaton__node_2__finite::node)__res;
}
...
class cmd_reader: public stdin_reader {
    int lineno;
public:
    lockserv_automaton__node_2__finite_repl &ivy;    

    cmd_reader(lockserv_automaton__node_2__finite_repl &_ivy) : ivy(_ivy) {
        lineno = 1;
        if (isatty(fdes()))
        ...
        try {
            ...
            ivy.__lock();
                ...
                if (action == "get_bool_grant_msg") {
                    ...
                    __ivy_out  << "= " << ivy.ext__get_bool_grant_msg(_arg<lockserv_automaton__node_2__finite::node>(args,0,2),_arg<bool>(args,1,2)) << std::endl;
                }
                else
                ...
                if (action == "get_bool_holds_lock") {
                    check_arity(args,2,action);
                    __ivy_out  << "= " << ivy.ext__get_bool_holds_lock(_arg<lockserv_automaton__node_2__finite::node>(args,0,2),_arg<bool>(args,1,2)) << std::endl;
                }
                else
    
                if (action == "get_bool_lock_msg") {
                    check_arity(args,2,action);
                    __ivy_out  << "= " << ivy.ext__get_bool_lock_msg(_arg<lockserv_automaton__node_2__finite::node>(args,0,2),_arg<bool>(args,1,2)) << std::endl;
                }
                else
    
                if (action == "get_bool_unlock_msg") {
                    check_arity(args,2,action);
                    __ivy_out  << "= " << ivy.ext__get_bool_unlock_msg(_arg<lockserv_automaton__node_2__finite::node>(args,0,2),_arg<bool>(args,1,2)) << std::endl;
                }
                else
    
                if (action == "get_grant_msg") {
                    check_arity(args,1,action);
                    __ivy_out  << "= " << ivy.ext__get_grant_msg(_arg<lockserv_automaton__node_2__finite::node>(args,0,2)) << std::endl;
                }
                else
                ...
                if (action == "get_holds_lock") {
                    check_arity(args,1,action);
                    __ivy_out  << "= " << ivy.ext__get_holds_lock(_arg<lockserv_automaton__node_2__finite::node>(args,0,2)) << std::endl;
                }
                else
    
                if (action == "get_lock_msg") {
                    check_arity(args,1,action);
                    __ivy_out  << "= " << ivy.ext__get_lock_msg(_arg<lockserv_automaton__node_2__finite::node>(args,0,2)) << std::endl;
                }
                else
    
                if (action == "get_unlock_msg") {
                    check_arity(args,1,action);
                    __ivy_out  << "= " << ivy.ext__get_unlock_msg(_arg<lockserv_automaton__node_2__finite::node>(args,0,2)) << std::endl;
                }
                else
    
                if (action == "lock") {
                    check_arity(args,1,action);
                    ivy.ext__lock(_arg<lockserv_automaton__node_2__finite::node>(args,0,2));
                }
                else
    
                if (action == "recv_grant") {
                    check_arity(args,1,action);
                    ivy.ext__recv_grant(_arg<lockserv_automaton__node_2__finite::node>(args,0,2));
                }
                else
    
                if (action == "recv_lock") {
                    check_arity(args,1,action);
                    ivy.ext__recv_lock(_arg<lockserv_automaton__node_2__finite::node>(args,0,2));
                }
                else
    
                if (action == "recv_unlock") {
                    check_arity(args,1,action);
                    ivy.ext__recv_unlock(_arg<lockserv_automaton__node_2__finite::node>(args,0,2));
                }
                else
    
                if (action == "unlock") {
                    check_arity(args,1,action);
                    ivy.ext__unlock(_arg<lockserv_automaton__node_2__finite::node>(args,0,2));
                }
                else
...
int main(int argc, char **argv){
    ...
    if (argc != 1){
        std::cerr << "usage: lockserv_automaton__node_2__finite \n";
        __ivy_exit(1);
    }
...
void ivy_exec_set_state(std::vector<std::string> state_values){
	...
	std::stringstream(state_values[3]) >> ivy_exec -> grant_msg[0];
	std::stringstream(state_values[4]) >> ivy_exec -> grant_msg[1];
	std::stringstream(state_values[5]) >> ivy_exec -> unlock_msg[0];
	std::stringstream(state_values[6]) >> ivy_exec -> unlock_msg[1];
	std::stringstream(state_values[7]) >> ivy_exec -> lock_msg[0];
	std::stringstream(state_values[8]) >> ivy_exec -> lock_msg[1];
}
