#include "simple-decentralized-lock.node_2.finite.h"
...
#include <cstdint>
#endif
typedef simple_decentralized_lock__node_2__finite ivy_class;
...
void simple_decentralized_lock__node_2__finite::install_reader(reader *r) {
    #ifdef _WIN32
...
void simple_decentralized_lock__node_2__finite::install_thread(reader *r) {
    install_reader(r);
}

void simple_decentralized_lock__node_2__finite::install_timer(timer *r) {
    #ifdef _WIN32
...
#ifdef _WIN32
    void simple_decentralized_lock__node_2__finite::__lock() { WaitForSingleObject(mutex,INFINITE); }
    void simple_decentralized_lock__node_2__finite::__unlock() { ReleaseMutex(mutex); }
#else
    void simple_decentralized_lock__node_2__finite::__lock() { pthread_mutex_lock(&mutex); }
    void simple_decentralized_lock__node_2__finite::__unlock() { pthread_mutex_unlock(&mutex); }
#endif
...
std::ostream &operator <<(std::ostream &s, const simple_decentralized_lock__node_2__finite::node &t);
template <>
simple_decentralized_lock__node_2__finite::node _arg<simple_decentralized_lock__node_2__finite::node>(std::vector<ivy_value> &args, unsigned idx, long long bound);
template <>
void  __ser<simple_decentralized_lock__node_2__finite::node>(ivy_ser &res, const simple_decentralized_lock__node_2__finite::node&);
template <>
void  __deser<simple_decentralized_lock__node_2__finite::node>(ivy_deser &inp, simple_decentralized_lock__node_2__finite::node &res);
int simple_decentralized_lock__node_2__finite::___ivy_choose(int rng,const char *name,int id) {
        return 0;
    }
struct ivy_nondet_except {}; // lauren-yrluo added
void simple_decentralized_lock__node_2__finite::__init(){
        bool __tmp0[2][2];
...
void simple_decentralized_lock__node_2__finite::ext__send(node src, node dst){
        ivy_assume(has_lock[src], "simple-decentralized-lock.node_2.finite.ivy: line 18");
        message[src][dst] = true;
        has_lock[src] = false;
}
void simple_decentralized_lock__node_2__finite::ext__recv(node src, node dst){
        ivy_assume(message[src][dst], "simple-decentralized-lock.node_2.finite.ivy: line 24");
        message[src][dst] = false;
        has_lock[dst] = true;
}
bool simple_decentralized_lock__node_2__finite::ext__get_message(node n0, node n1){
    bool x;
    x = message[n0][n1];
    return x;
}
bool simple_decentralized_lock__node_2__finite::ext__get_bool_message(node n0, node n1, bool result){
    bool x;
    x = (message[n0][n1] == result);
    return x;
}
simple_decentralized_lock__node_2__finite::node simple_decentralized_lock__node_2__finite::ext__get_start_node(){
    simple_decentralized_lock__node_2__finite::node x;
    x = start_node;
    return x;
}
bool simple_decentralized_lock__node_2__finite::ext__get_bool_start_node(node result){
    bool x;
    x = (start_node == result);
    return x;
}
bool simple_decentralized_lock__node_2__finite::ext__get_has_lock(node n0){
    bool x;
    x = has_lock[n0];
    return x;
}
bool simple_decentralized_lock__node_2__finite::ext__get_bool_has_lock(node n0, bool result){
    bool x;
    x = (has_lock[n0] == result);
    return x;
}
void simple_decentralized_lock__node_2__finite::__tick(int __timeout){
}
simple_decentralized_lock__node_2__finite::simple_decentralized_lock__node_2__finite(){
#ifdef _WIN32
...
std::ostream &operator <<(std::ostream &s, const simple_decentralized_lock__node_2__finite::node &t){
    if (t == simple_decentralized_lock__node_2__finite::node0) s<<"node0";
    if (t == simple_decentralized_lock__node_2__finite::node1) s<<"node1";
    return s;
}
template <>
void  __ser<simple_decentralized_lock__node_2__finite::node>(ivy_ser &res, const simple_decentralized_lock__node_2__finite::node&t){
    __ser(res,(int)t);
}
...
int ask_ret(long long bound) {
    ...
    class simple_decentralized_lock__node_2__finite_repl : public simple_decentralized_lock__node_2__finite {
    ...
    virtual void ivy_assume(bool truth,const char *msg){
        ...
        simple_decentralized_lock__node_2__finite_repl() : simple_decentralized_lock__node_2__finite(){}
...
simple_decentralized_lock__node_2__finite::node _arg<simple_decentralized_lock__node_2__finite::node>(std::vector<ivy_value> &args, unsigned idx, long long bound){
    ...
    if(arg.atom == "node0") return simple_decentralized_lock__node_2__finite::node0;
    if(arg.atom == "node1") return simple_decentralized_lock__node_2__finite::node1;
...
void __deser<simple_decentralized_lock__node_2__finite::node>(ivy_deser &inp, simple_decentralized_lock__node_2__finite::node &res){
    int __res;
    __deser(inp,__res);
    res = (simple_decentralized_lock__node_2__finite::node)__res;
}
...
class cmd_reader: public stdin_reader {
    int lineno;
public:
    simple_decentralized_lock__node_2__finite_repl &ivy;    

    cmd_reader(simple_decentralized_lock__node_2__finite_repl &_ivy) : ivy(_ivy) {
        lineno = 1;
        if (isatty(fdes()))
        ...
        try {
            ...
            ivy.__lock();
                ...
                if (action == "get_bool_has_lock") {
                    ...
                    __ivy_out  << "= " << ivy.ext__get_bool_has_lock(_arg<simple_decentralized_lock__node_2__finite::node>(args,0,2),_arg<bool>(args,1,2)) << std::endl;
                }
                else
    
                if (action == "get_bool_message") {
                    check_arity(args,3,action);
                    __ivy_out  << "= " << ivy.ext__get_bool_message(_arg<simple_decentralized_lock__node_2__finite::node>(args,0,2),_arg<simple_decentralized_lock__node_2__finite::node>(args,1,2),_arg<bool>(args,2,2)) << std::endl;
                }
                else
    
                if (action == "get_bool_start_node") {
                    check_arity(args,1,action);
                    __ivy_out  << "= " << ivy.ext__get_bool_start_node(_arg<simple_decentralized_lock__node_2__finite::node>(args,0,2)) << std::endl;
                }
                else
    
                if (action == "get_has_lock") {
                    check_arity(args,1,action);
                    __ivy_out  << "= " << ivy.ext__get_has_lock(_arg<simple_decentralized_lock__node_2__finite::node>(args,0,2)) << std::endl;
                }
                else
    
                if (action == "get_message") {
                    check_arity(args,2,action);
                    __ivy_out  << "= " << ivy.ext__get_message(_arg<simple_decentralized_lock__node_2__finite::node>(args,0,2),_arg<simple_decentralized_lock__node_2__finite::node>(args,1,2)) << std::endl;
                }
                else
                ...
                if (action == "recv") {
                    check_arity(args,2,action);
                    ivy.ext__recv(_arg<simple_decentralized_lock__node_2__finite::node>(args,0,2),_arg<simple_decentralized_lock__node_2__finite::node>(args,1,2));
                }
                else
    
                if (action == "send") {
                    check_arity(args,2,action);
                    ivy.ext__send(_arg<simple_decentralized_lock__node_2__finite::node>(args,0,2),_arg<simple_decentralized_lock__node_2__finite::node>(args,1,2));
                }
                else
...
int main(int argc, char **argv){
    ...
    if (argc != 1){
        std::cerr << "usage: simple_decentralized_lock__node_2__finite \n";
        __ivy_exit(1);
    }
...
#include <vector>
simple_decentralized_lock__node_2__finite_repl * ivy_exec;
cmd_reader* ivy_exec_cr;
std::ostringstream ivy_exec_stream;

void ivy_exec_init(){
	__ivy_out.basic_ios<char>::rdbuf(ivy_exec_stream.rdbuf());
	ivy_exec = new simple_decentralized_lock__node_2__finite_repl;
	ivy_exec -> __unlock();
...
void ivy_exec_set_state(std::vector<std::string> state_values){
	...
	std::stringstream(state_values[2]) >> ivy_exec -> message[1][0];
	std::stringstream(state_values[3]) >> ivy_exec -> message[1][1];
	std::stringstream(state_values[4]) >> ivy_exec -> has_lock[0];
	std::stringstream(state_values[5]) >> ivy_exec -> has_lock[1];
	ivy_value arg; arg.pos = 0; arg.atom = state_values[6];
	std::vector<ivy_value> args; args.push_back(arg);
	ivy_exec -> start_node = _arg<simple_decentralized_lock__node_2__finite::node>(args, 0, 0);
}
