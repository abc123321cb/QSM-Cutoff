#include "lock_server.server_1_client_2.finite.h"
...
#include <cstdint>
#endif
typedef lock_server__server_1_client_2__finite ivy_class;
...
void lock_server__server_1_client_2__finite::install_reader(reader *r) {
    #ifdef _WIN32
...
void lock_server__server_1_client_2__finite::install_thread(reader *r) {
    install_reader(r);
}

void lock_server__server_1_client_2__finite::install_timer(timer *r) {
    #ifdef _WIN32
...
#ifdef _WIN32
    void lock_server__server_1_client_2__finite::__lock() { WaitForSingleObject(mutex,INFINITE); }
    void lock_server__server_1_client_2__finite::__unlock() { ReleaseMutex(mutex); }
#else
    void lock_server__server_1_client_2__finite::__lock() { pthread_mutex_lock(&mutex); }
    void lock_server__server_1_client_2__finite::__unlock() { pthread_mutex_unlock(&mutex); }
#endif
...
std::ostream &operator <<(std::ostream &s, const lock_server__server_1_client_2__finite::client &t);
template <>
lock_server__server_1_client_2__finite::client _arg<lock_server__server_1_client_2__finite::client>(std::vector<ivy_value> &args, unsigned idx, long long bound);
template <>
void  __ser<lock_server__server_1_client_2__finite::client>(ivy_ser &res, const lock_server__server_1_client_2__finite::client&);
template <>
void  __deser<lock_server__server_1_client_2__finite::client>(ivy_deser &inp, lock_server__server_1_client_2__finite::client &res);
std::ostream &operator <<(std::ostream &s, const lock_server__server_1_client_2__finite::server &t);
template <>
lock_server__server_1_client_2__finite::server _arg<lock_server__server_1_client_2__finite::server>(std::vector<ivy_value> &args, unsigned idx, long long bound);
template <>
void  __ser<lock_server__server_1_client_2__finite::server>(ivy_ser &res, const lock_server__server_1_client_2__finite::server&);
template <>
void  __deser<lock_server__server_1_client_2__finite::server>(ivy_deser &inp, lock_server__server_1_client_2__finite::server &res);
int lock_server__server_1_client_2__finite::___ivy_choose(int rng,const char *name,int id) {
        return 0;
    }
struct ivy_nondet_except {}; // lauren-yrluo added
void lock_server__server_1_client_2__finite::__init(){
        bool __tmp0[1];
        for (int Y = 0; Y < 1; Y++) {
            __tmp0[Y] = true;
        }
...
void lock_server__server_1_client_2__finite::ext__connect(client c, server s){
        ivy_assume(semaphore[s], "lock_server.server_1_client_2.finite.ivy: line 19");
        link[c][s] = true;
        semaphore[s] = false;
}
void lock_server__server_1_client_2__finite::ext__disconnect(client c, server s){
        ivy_assume(link[c][s], "lock_server.server_1_client_2.finite.ivy: line 25");
        link[c][s] = false;
        semaphore[s] = true;
}
bool lock_server__server_1_client_2__finite::ext__get_semaphore(server s0){
    bool x;
    x = semaphore[s0];
    return x;
}
bool lock_server__server_1_client_2__finite::ext__get_bool_semaphore(server s0, bool result){
    bool x;
    x = (semaphore[s0] == result);
    return x;
}
bool lock_server__server_1_client_2__finite::ext__get_link(client c0, server s1){
    bool x;
    x = link[c0][s1];
    return x;
}
bool lock_server__server_1_client_2__finite::ext__get_bool_link(client c0, server s1, bool result){
    bool x;
    x = (link[c0][s1] == result);
    return x;
}
void lock_server__server_1_client_2__finite::__tick(int __timeout){
}
lock_server__server_1_client_2__finite::lock_server__server_1_client_2__finite(){
#ifdef _WIN32
...
std::ostream &operator <<(std::ostream &s, const lock_server__server_1_client_2__finite::client &t){
    if (t == lock_server__server_1_client_2__finite::client0) s<<"client0";
    if (t == lock_server__server_1_client_2__finite::client1) s<<"client1";
    return s;
}
template <>
void  __ser<lock_server__server_1_client_2__finite::client>(ivy_ser &res, const lock_server__server_1_client_2__finite::client&t){
    __ser(res,(int)t);
}
std::ostream &operator <<(std::ostream &s, const lock_server__server_1_client_2__finite::server &t){
    if (t == lock_server__server_1_client_2__finite::server0) s<<"server0";
    return s;
}
template <>
void  __ser<lock_server__server_1_client_2__finite::server>(ivy_ser &res, const lock_server__server_1_client_2__finite::server&t){
    __ser(res,(int)t);
}
...
int ask_ret(long long bound) {
    ...
    class lock_server__server_1_client_2__finite_repl : public lock_server__server_1_client_2__finite {
    ...
    virtual void ivy_assume(bool truth,const char *msg){
        ...
        lock_server__server_1_client_2__finite_repl() : lock_server__server_1_client_2__finite(){}
...
lock_server__server_1_client_2__finite::client _arg<lock_server__server_1_client_2__finite::client>(std::vector<ivy_value> &args, unsigned idx, long long bound){
    ...
    if(arg.atom == "client0") return lock_server__server_1_client_2__finite::client0;
    if(arg.atom == "client1") return lock_server__server_1_client_2__finite::client1;
...
void __deser<lock_server__server_1_client_2__finite::client>(ivy_deser &inp, lock_server__server_1_client_2__finite::client &res){
    int __res;
    __deser(inp,__res);
    res = (lock_server__server_1_client_2__finite::client)__res;
}
template <>
lock_server__server_1_client_2__finite::server _arg<lock_server__server_1_client_2__finite::server>(std::vector<ivy_value> &args, unsigned idx, long long bound){
    ivy_value &arg = args[idx];
    if (arg.atom.size() == 0 || arg.fields.size() != 0) throw out_of_bounds(idx,arg.pos);
    if(arg.atom == "server0") return lock_server__server_1_client_2__finite::server0;
    throw out_of_bounds("bad value: " + arg.atom,arg.pos);
}
template <>
void __deser<lock_server__server_1_client_2__finite::server>(ivy_deser &inp, lock_server__server_1_client_2__finite::server &res){
    int __res;
    __deser(inp,__res);
    res = (lock_server__server_1_client_2__finite::server)__res;
}
...
class cmd_reader: public stdin_reader {
    int lineno;
public:
    lock_server__server_1_client_2__finite_repl &ivy;    

    cmd_reader(lock_server__server_1_client_2__finite_repl &_ivy) : ivy(_ivy) {
        lineno = 1;
        if (isatty(fdes()))
        ...
        try {
            ...
            ivy.__lock();
                ...
                if (action == "connect") {
                    ...
                    ivy.ext__connect(_arg<lock_server__server_1_client_2__finite::client>(args,0,2),_arg<lock_server__server_1_client_2__finite::server>(args,1,1));
                }
                else
    
                if (action == "disconnect") {
                    check_arity(args,2,action);
                    ivy.ext__disconnect(_arg<lock_server__server_1_client_2__finite::client>(args,0,2),_arg<lock_server__server_1_client_2__finite::server>(args,1,1));
                }
                else
    
                if (action == "get_bool_link") {
                    check_arity(args,3,action);
                    __ivy_out  << "= " << ivy.ext__get_bool_link(_arg<lock_server__server_1_client_2__finite::client>(args,0,2),_arg<lock_server__server_1_client_2__finite::server>(args,1,1),_arg<bool>(args,2,2)) << std::endl;
                }
                else
    
                if (action == "get_bool_semaphore") {
                    check_arity(args,2,action);
                    __ivy_out  << "= " << ivy.ext__get_bool_semaphore(_arg<lock_server__server_1_client_2__finite::server>(args,0,1),_arg<bool>(args,1,2)) << std::endl;
                }
                else
    
                if (action == "get_link") {
                    check_arity(args,2,action);
                    __ivy_out  << "= " << ivy.ext__get_link(_arg<lock_server__server_1_client_2__finite::client>(args,0,2),_arg<lock_server__server_1_client_2__finite::server>(args,1,1)) << std::endl;
                }
                else
    
                if (action == "get_semaphore") {
                    check_arity(args,1,action);
                    __ivy_out  << "= " << ivy.ext__get_semaphore(_arg<lock_server__server_1_client_2__finite::server>(args,0,1)) << std::endl;
                }
                else
...
int main(int argc, char **argv){
    ...
    if (argc != 1){
        std::cerr << "usage: lock_server__server_1_client_2__finite \n";
        __ivy_exit(1);
    }
...
#include <vector>
lock_server__server_1_client_2__finite_repl * ivy_exec;
cmd_reader* ivy_exec_cr;
std::ostringstream ivy_exec_stream;

void ivy_exec_init(){
	__ivy_out.basic_ios<char>::rdbuf(ivy_exec_stream.rdbuf());
	ivy_exec = new lock_server__server_1_client_2__finite_repl;
	ivy_exec -> __unlock();
