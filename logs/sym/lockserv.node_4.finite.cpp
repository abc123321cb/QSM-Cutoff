#include "lockserv.node_4.finite.h"
...
#include <cstdint>
#endif
typedef lockserv__node_4__finite ivy_class;
...
void lockserv__node_4__finite::install_reader(reader *r) {
    #ifdef _WIN32
...
void lockserv__node_4__finite::install_thread(reader *r) {
    install_reader(r);
}

void lockserv__node_4__finite::install_timer(timer *r) {
    #ifdef _WIN32
...
#ifdef _WIN32
    void lockserv__node_4__finite::__lock() { WaitForSingleObject(mutex,INFINITE); }
    void lockserv__node_4__finite::__unlock() { ReleaseMutex(mutex); }
#else
    void lockserv__node_4__finite::__lock() { pthread_mutex_lock(&mutex); }
    void lockserv__node_4__finite::__unlock() { pthread_mutex_unlock(&mutex); }
#endif
...
std::ostream &operator <<(std::ostream &s, const lockserv__node_4__finite::node &t);
template <>
lockserv__node_4__finite::node _arg<lockserv__node_4__finite::node>(std::vector<ivy_value> &args, unsigned idx, long long bound);
template <>
void  __ser<lockserv__node_4__finite::node>(ivy_ser &res, const lockserv__node_4__finite::node&);
template <>
void  __deser<lockserv__node_4__finite::node>(ivy_deser &inp, lockserv__node_4__finite::node &res);
int lockserv__node_4__finite::___ivy_choose(int rng,const char *name,int id) {
        return 0;
    }
struct ivy_nondet_except {}; // lauren-yrluo added
void lockserv__node_4__finite::__init(){
        bool __tmp0[4];
        for (int N = 0; N < 4; N++) {
            __tmp0[N] = false;
        }
        ...
        for (int N = 0; N < 4; N++) {
            holds_lock[N] = __tmp3[N];
        }
        server_holds_lock = true;
}
void lockserv__node_4__finite::ext__send_lock(node n){
    lock_msg[n] = true;
}
void lockserv__node_4__finite::ext__recv_lock(node n){
        ivy_assume(server_holds_lock, "lockserv.node_4.finite.ivy: line 24");
        ivy_assume(lock_msg[n], "lockserv.node_4.finite.ivy: line 25");
        server_holds_lock = false;
        bool __tmp4[4];
        for (int N = 0; N < 4; N++) {
            __tmp4[N] = (lock_msg[N] && !(N == n));
        }
        for (int N = 0; N < 4; N++) {
            lock_msg[N] = __tmp4[N];
        }
...
void lockserv__node_4__finite::ext__recv_grant(node n){
        ivy_assume(grant_msg[n], "lockserv.node_4.finite.ivy: line 33");
        bool __tmp5[4];
        for (int N = 0; N < 4; N++) {
            __tmp5[N] = (grant_msg[N] && !(N == n));
        }
        for (int N = 0; N < 4; N++) {
            grant_msg[N] = __tmp5[N];
        }
        holds_lock[n] = true;
}
void lockserv__node_4__finite::ext__unlock(node n){
        ivy_assume(holds_lock[n], "lockserv.node_4.finite.ivy: line 40");
        bool __tmp6[4];
        for (int N = 0; N < 4; N++) {
            __tmp6[N] = (holds_lock[N] && !(N == n));
        }
        for (int N = 0; N < 4; N++) {
            holds_lock[N] = __tmp6[N];
        }
        unlock_msg[n] = true;
}
void lockserv__node_4__finite::ext__recv_unlock(node n){
        ivy_assume(unlock_msg[n], "lockserv.node_4.finite.ivy: line 47");
        bool __tmp7[4];
        for (int N = 0; N < 4; N++) {
            __tmp7[N] = (unlock_msg[N] && !(N == n));
        }
        for (int N = 0; N < 4; N++) {
            unlock_msg[N] = __tmp7[N];
        }
        server_holds_lock = true;
}
bool lockserv__node_4__finite::ext__get_grant_msg(node n0){
    bool x;
    x = grant_msg[n0];
    return x;
}
bool lockserv__node_4__finite::ext__get_bool_grant_msg(node n0, bool result){
    bool x;
    x = (grant_msg[n0] == result);
    return x;
}
bool lockserv__node_4__finite::ext__get_holds_lock(node n0){
    bool x;
    x = holds_lock[n0];
    return x;
}
bool lockserv__node_4__finite::ext__get_bool_holds_lock(node n0, bool result){
    bool x;
    x = (holds_lock[n0] == result);
    return x;
}
bool lockserv__node_4__finite::ext__get_lock_msg(node n0){
    bool x;
    x = lock_msg[n0];
    return x;
}
bool lockserv__node_4__finite::ext__get_bool_lock_msg(node n0, bool result){
    bool x;
    x = (lock_msg[n0] == result);
    return x;
}
bool lockserv__node_4__finite::ext__get_server_holds_lock(){
    bool x;
    x = server_holds_lock;
    return x;
}
bool lockserv__node_4__finite::ext__get_bool_server_holds_lock(bool result){
    bool x;
    x = (server_holds_lock == result);
    return x;
}
bool lockserv__node_4__finite::ext__get_unlock_msg(node n0){
    bool x;
    x = unlock_msg[n0];
    return x;
}
bool lockserv__node_4__finite::ext__get_bool_unlock_msg(node n0, bool result){
    bool x;
    x = (unlock_msg[n0] == result);
    return x;
}
void lockserv__node_4__finite::__tick(int __timeout){
}
lockserv__node_4__finite::lockserv__node_4__finite(){
#ifdef _WIN32
...
lock_msg[X__0] = (bool)___ivy_choose(0,"init",0);
}
for (node X__0 = (node)0; (int) X__0 < 4; X__0 = (node)(((int)X__0) + 1)) {
grant_msg[X__0] = (bool)___ivy_choose(0,"init",0);
}
for (node X__0 = (node)0; (int) X__0 < 4; X__0 = (node)(((int)X__0) + 1)) {
unlock_msg[X__0] = (bool)___ivy_choose(0,"init",0);
}
for (node X__0 = (node)0; (int) X__0 < 4; X__0 = (node)(((int)X__0) + 1)) {
holds_lock[X__0] = (bool)___ivy_choose(0,"init",0);
}
server_holds_lock = (bool)___ivy_choose(0,"init",0);
}
lockserv__node_4__finite::~lockserv__node_4__finite(){
...
std::ostream &operator <<(std::ostream &s, const lockserv__node_4__finite::node &t){
    if (t == lockserv__node_4__finite::node0) s<<"node0";
    if (t == lockserv__node_4__finite::node1) s<<"node1";
    if (t == lockserv__node_4__finite::node2) s<<"node2";
    if (t == lockserv__node_4__finite::node3) s<<"node3";
    return s;
}
template <>
void  __ser<lockserv__node_4__finite::node>(ivy_ser &res, const lockserv__node_4__finite::node&t){
    __ser(res,(int)t);
}
...
int ask_ret(long long bound) {
    ...
    class lockserv__node_4__finite_repl : public lockserv__node_4__finite {
    ...
    virtual void ivy_assume(bool truth,const char *msg){
        ...
        lockserv__node_4__finite_repl() : lockserv__node_4__finite(){}
...
lockserv__node_4__finite::node _arg<lockserv__node_4__finite::node>(std::vector<ivy_value> &args, unsigned idx, long long bound){
    ...
    if(arg.atom == "node0") return lockserv__node_4__finite::node0;
    if(arg.atom == "node1") return lockserv__node_4__finite::node1;
    if(arg.atom == "node2") return lockserv__node_4__finite::node2;
    if(arg.atom == "node3") return lockserv__node_4__finite::node3;
...
void __deser<lockserv__node_4__finite::node>(ivy_deser &inp, lockserv__node_4__finite::node &res){
    int __res;
    __deser(inp,__res);
    res = (lockserv__node_4__finite::node)__res;
}
...
class cmd_reader: public stdin_reader {
    int lineno;
public:
    lockserv__node_4__finite_repl &ivy;    

    cmd_reader(lockserv__node_4__finite_repl &_ivy) : ivy(_ivy) {
        lineno = 1;
        if (isatty(fdes()))
    ...
    virtual void process(const std::string &cmd) {
        ...
        try {
            ...
            ivy.__lock();
                ...
                if (action == "get_bool_grant_msg") {
                    check_arity(args,2,action);
                    __ivy_out  << "= " << ivy.ext__get_bool_grant_msg(_arg<lockserv__node_4__finite::node>(args,0,4),_arg<bool>(args,1,2)) << std::endl;
                }
                else
    
                if (action == "get_bool_holds_lock") {
                    check_arity(args,2,action);
                    __ivy_out  << "= " << ivy.ext__get_bool_holds_lock(_arg<lockserv__node_4__finite::node>(args,0,4),_arg<bool>(args,1,2)) << std::endl;
                }
                else
    
                if (action == "get_bool_lock_msg") {
                    check_arity(args,2,action);
                    __ivy_out  << "= " << ivy.ext__get_bool_lock_msg(_arg<lockserv__node_4__finite::node>(args,0,4),_arg<bool>(args,1,2)) << std::endl;
                }
                else
    
                if (action == "get_bool_server_holds_lock") {
                    check_arity(args,1,action);
                    __ivy_out  << "= " << ivy.ext__get_bool_server_holds_lock(_arg<bool>(args,0,2)) << std::endl;
                }
                else
    
                if (action == "get_bool_unlock_msg") {
                    check_arity(args,2,action);
                    __ivy_out  << "= " << ivy.ext__get_bool_unlock_msg(_arg<lockserv__node_4__finite::node>(args,0,4),_arg<bool>(args,1,2)) << std::endl;
                }
                else
    
                if (action == "get_grant_msg") {
                    check_arity(args,1,action);
                    __ivy_out  << "= " << ivy.ext__get_grant_msg(_arg<lockserv__node_4__finite::node>(args,0,4)) << std::endl;
                }
                else
    
                if (action == "get_holds_lock") {
                    check_arity(args,1,action);
                    __ivy_out  << "= " << ivy.ext__get_holds_lock(_arg<lockserv__node_4__finite::node>(args,0,4)) << std::endl;
                }
                else
    
                if (action == "get_lock_msg") {
                    check_arity(args,1,action);
                    __ivy_out  << "= " << ivy.ext__get_lock_msg(_arg<lockserv__node_4__finite::node>(args,0,4)) << std::endl;
                }
                else
    
                if (action == "get_server_holds_lock") {
                    check_arity(args,0,action);
                    __ivy_out  << "= " << ivy.ext__get_server_holds_lock() << std::endl;
                }
                else
    
                if (action == "get_unlock_msg") {
                    check_arity(args,1,action);
                    __ivy_out  << "= " << ivy.ext__get_unlock_msg(_arg<lockserv__node_4__finite::node>(args,0,4)) << std::endl;
                }
                else
    
                if (action == "recv_grant") {
                    check_arity(args,1,action);
                    ivy.ext__recv_grant(_arg<lockserv__node_4__finite::node>(args,0,4));
                }
                else
    
                if (action == "recv_lock") {
                    check_arity(args,1,action);
                    ivy.ext__recv_lock(_arg<lockserv__node_4__finite::node>(args,0,4));
                }
                else
    
                if (action == "recv_unlock") {
                    check_arity(args,1,action);
                    ivy.ext__recv_unlock(_arg<lockserv__node_4__finite::node>(args,0,4));
                }
                else
    
                if (action == "send_lock") {
                    check_arity(args,1,action);
                    ivy.ext__send_lock(_arg<lockserv__node_4__finite::node>(args,0,4));
                }
                else
    
                if (action == "unlock") {
                    check_arity(args,1,action);
                    ivy.ext__unlock(_arg<lockserv__node_4__finite::node>(args,0,4));
                }
                else
...
int main(int argc, char **argv){
    ...
    if (argc != 1){
        std::cerr << "usage: lockserv__node_4__finite \n";
        __ivy_exit(1);
    }
...
void ivy_exec_set_state(std::vector<std::string> state_values){
	std::stringstream(state_values[0]) >> ivy_exec -> server_holds_lock;
	std::stringstream(state_values[1]) >> ivy_exec -> holds_lock[0];
	std::stringstream(state_values[2]) >> ivy_exec -> holds_lock[1];
	std::stringstream(state_values[3]) >> ivy_exec -> holds_lock[2];
	std::stringstream(state_values[4]) >> ivy_exec -> holds_lock[3];
	std::stringstream(state_values[5]) >> ivy_exec -> unlock_msg[0];
	std::stringstream(state_values[6]) >> ivy_exec -> unlock_msg[1];
	std::stringstream(state_values[7]) >> ivy_exec -> unlock_msg[2];
	std::stringstream(state_values[8]) >> ivy_exec -> unlock_msg[3];
	std::stringstream(state_values[9]) >> ivy_exec -> grant_msg[0];
	std::stringstream(state_values[10]) >> ivy_exec -> grant_msg[1];
	std::stringstream(state_values[11]) >> ivy_exec -> grant_msg[2];
	std::stringstream(state_values[12]) >> ivy_exec -> grant_msg[3];
	std::stringstream(state_values[13]) >> ivy_exec -> lock_msg[0];
	std::stringstream(state_values[14]) >> ivy_exec -> lock_msg[1];
	std::stringstream(state_values[15]) >> ivy_exec -> lock_msg[2];
	std::stringstream(state_values[16]) >> ivy_exec -> lock_msg[3];
}
