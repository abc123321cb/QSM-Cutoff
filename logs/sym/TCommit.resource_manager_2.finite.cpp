#include "TCommit.resource_manager_2.finite.h"
...
#include <cstdint>
#endif
typedef TCommit__resource_manager_2__finite ivy_class;
...
void TCommit__resource_manager_2__finite::install_reader(reader *r) {
    #ifdef _WIN32
...
void TCommit__resource_manager_2__finite::install_thread(reader *r) {
    install_reader(r);
}

void TCommit__resource_manager_2__finite::install_timer(timer *r) {
    #ifdef _WIN32
...
#ifdef _WIN32
    void TCommit__resource_manager_2__finite::__lock() { WaitForSingleObject(mutex,INFINITE); }
    void TCommit__resource_manager_2__finite::__unlock() { ReleaseMutex(mutex); }
#else
    void TCommit__resource_manager_2__finite::__lock() { pthread_mutex_lock(&mutex); }
    void TCommit__resource_manager_2__finite::__unlock() { pthread_mutex_unlock(&mutex); }
#endif
...
std::ostream &operator <<(std::ostream &s, const TCommit__resource_manager_2__finite::resource_manager &t);
template <>
TCommit__resource_manager_2__finite::resource_manager _arg<TCommit__resource_manager_2__finite::resource_manager>(std::vector<ivy_value> &args, unsigned idx, long long bound);
template <>
void  __ser<TCommit__resource_manager_2__finite::resource_manager>(ivy_ser &res, const TCommit__resource_manager_2__finite::resource_manager&);
template <>
void  __deser<TCommit__resource_manager_2__finite::resource_manager>(ivy_deser &inp, TCommit__resource_manager_2__finite::resource_manager &res);
int TCommit__resource_manager_2__finite::___ivy_choose(int rng,const char *name,int id) {
        return 0;
    }
struct ivy_nondet_except {}; // lauren-yrluo added
void TCommit__resource_manager_2__finite::__init(){
        bool __tmp0[2];
        for (int R = 0; R < 2; R++) {
            __tmp0[R] = true;
        }
...
void TCommit__resource_manager_2__finite::ext__prepare(resource_manager rm){
        ivy_assume(working[rm], "TCommit.resource_manager_2.finite.ivy: line 18");
        working[rm] = false;
        prepared[rm] = true;
        committed[rm] = false;
        aborted[rm] = false;
}
void TCommit__resource_manager_2__finite::ext__decide_commit(resource_manager rm){
        ivy_assume(prepared[rm], "TCommit.resource_manager_2.finite.ivy: line 26");
        int __tmp4;
        __tmp4 = 1;
        for (int R = 0; R < 2; R++) {
            if (!(prepared[R] || committed[R])) __tmp4 = 0;
        }
        ivy_assume(__tmp4, "TCommit.resource_manager_2.finite.ivy: line 27");
        working[rm] = false;
        prepared[rm] = false;
        committed[rm] = true;
        aborted[rm] = false;
}
void TCommit__resource_manager_2__finite::ext__decide_abort(resource_manager rm){
        ivy_assume((working[rm] || prepared[rm]), "TCommit.resource_manager_2.finite.ivy: line 35");
        int __tmp5;
        __tmp5 = 1;
        for (int R = 0; R < 2; R++) {
            if (!!committed[R]) __tmp5 = 0;
        }
        ivy_assume(__tmp5, "TCommit.resource_manager_2.finite.ivy: line 36");
        working[rm] = false;
        prepared[rm] = false;
        committed[rm] = false;
        aborted[rm] = true;
}
bool TCommit__resource_manager_2__finite::ext__get_aborted(resource_manager r0){
    bool x;
    x = aborted[r0];
    return x;
}
...
bool TCommit__resource_manager_2__finite::ext__get_bool_prepared(resource_manager r0, bool result){
    bool x;
...
TCommit__resource_manager_2__finite::TCommit__resource_manager_2__finite(){
#ifdef _WIN32
...
std::ostream &operator <<(std::ostream &s, const TCommit__resource_manager_2__finite::resource_manager &t){
    if (t == TCommit__resource_manager_2__finite::resource_manager0) s<<"resource_manager0";
    if (t == TCommit__resource_manager_2__finite::resource_manager1) s<<"resource_manager1";
    return s;
}
template <>
void  __ser<TCommit__resource_manager_2__finite::resource_manager>(ivy_ser &res, const TCommit__resource_manager_2__finite::resource_manager&t){
    __ser(res,(int)t);
}
...
int ask_ret(long long bound) {
    ...
    class TCommit__resource_manager_2__finite_repl : public TCommit__resource_manager_2__finite {
    ...
    virtual void ivy_assume(bool truth,const char *msg){
        ...
        TCommit__resource_manager_2__finite_repl() : TCommit__resource_manager_2__finite(){}
...
TCommit__resource_manager_2__finite::resource_manager _arg<TCommit__resource_manager_2__finite::resource_manager>(std::vector<ivy_value> &args, unsigned idx, long long bound){
    ...
    if(arg.atom == "resource_manager0") return TCommit__resource_manager_2__finite::resource_manager0;
    if(arg.atom == "resource_manager1") return TCommit__resource_manager_2__finite::resource_manager1;
...
void __deser<TCommit__resource_manager_2__finite::resource_manager>(ivy_deser &inp, TCommit__resource_manager_2__finite::resource_manager &res){
    int __res;
    __deser(inp,__res);
    res = (TCommit__resource_manager_2__finite::resource_manager)__res;
}
...
class cmd_reader: public stdin_reader {
    int lineno;
public:
    TCommit__resource_manager_2__finite_repl &ivy;    

    cmd_reader(TCommit__resource_manager_2__finite_repl &_ivy) : ivy(_ivy) {
        lineno = 1;
        if (isatty(fdes()))
        ...
        try {
            ...
            ivy.__lock();
                ...
                if (action == "decide_abort") {
                    ...
                    ivy.ext__decide_abort(_arg<TCommit__resource_manager_2__finite::resource_manager>(args,0,2));
                }
                else
    
                if (action == "decide_commit") {
                    check_arity(args,1,action);
                    ivy.ext__decide_commit(_arg<TCommit__resource_manager_2__finite::resource_manager>(args,0,2));
                }
                else
    
                if (action == "get_aborted") {
                    check_arity(args,1,action);
                    __ivy_out  << "= " << ivy.ext__get_aborted(_arg<TCommit__resource_manager_2__finite::resource_manager>(args,0,2)) << std::endl;
                }
                else
    
                if (action == "get_bool_aborted") {
                    check_arity(args,2,action);
                    __ivy_out  << "= " << ivy.ext__get_bool_aborted(_arg<TCommit__resource_manager_2__finite::resource_manager>(args,0,2),_arg<bool>(args,1,2)) << std::endl;
                }
                else
    
                if (action == "get_bool_committed") {
                    check_arity(args,2,action);
                    __ivy_out  << "= " << ivy.ext__get_bool_committed(_arg<TCommit__resource_manager_2__finite::resource_manager>(args,0,2),_arg<bool>(args,1,2)) << std::endl;
                }
                else
    
                if (action == "get_bool_prepared") {
                    check_arity(args,2,action);
                    __ivy_out  << "= " << ivy.ext__get_bool_prepared(_arg<TCommit__resource_manager_2__finite::resource_manager>(args,0,2),_arg<bool>(args,1,2)) << std::endl;
                }
                else
    
                if (action == "get_bool_working") {
                    check_arity(args,2,action);
                    __ivy_out  << "= " << ivy.ext__get_bool_working(_arg<TCommit__resource_manager_2__finite::resource_manager>(args,0,2),_arg<bool>(args,1,2)) << std::endl;
                }
                else
    
                if (action == "get_committed") {
                    check_arity(args,1,action);
                    __ivy_out  << "= " << ivy.ext__get_committed(_arg<TCommit__resource_manager_2__finite::resource_manager>(args,0,2)) << std::endl;
                }
                else
    
                if (action == "get_prepared") {
                    check_arity(args,1,action);
                    __ivy_out  << "= " << ivy.ext__get_prepared(_arg<TCommit__resource_manager_2__finite::resource_manager>(args,0,2)) << std::endl;
                }
                else
    
                if (action == "get_working") {
                    check_arity(args,1,action);
                    __ivy_out  << "= " << ivy.ext__get_working(_arg<TCommit__resource_manager_2__finite::resource_manager>(args,0,2)) << std::endl;
                }
                else
    
                if (action == "prepare") {
                    check_arity(args,1,action);
                    ivy.ext__prepare(_arg<TCommit__resource_manager_2__finite::resource_manager>(args,0,2));
                }
                else
...
int main(int argc, char **argv){
    ...
    if (argc != 1){
        std::cerr << "usage: TCommit__resource_manager_2__finite \n";
        __ivy_exit(1);
    }
...
void ivy_exec_set_state(std::vector<std::string> state_values){
	...
	std::stringstream(state_values[2]) >> ivy_exec -> prepared[0];
	std::stringstream(state_values[3]) >> ivy_exec -> prepared[1];
	std::stringstream(state_values[4]) >> ivy_exec -> committed[0];
	std::stringstream(state_values[5]) >> ivy_exec -> committed[1];
