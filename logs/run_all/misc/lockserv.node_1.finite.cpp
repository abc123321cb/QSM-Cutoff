#include "lockserv.node_1.finite.h"
...
#include <cstdint>
#endif
typedef lockserv__node_1__finite ivy_class;
...
void lockserv__node_1__finite::install_reader(reader *r) {
    #ifdef _WIN32
...
void lockserv__node_1__finite::install_thread(reader *r) {
    install_reader(r);
}

void lockserv__node_1__finite::install_timer(timer *r) {
    #ifdef _WIN32
...
#ifdef _WIN32
    void lockserv__node_1__finite::__lock() { WaitForSingleObject(mutex,INFINITE); }
    void lockserv__node_1__finite::__unlock() { ReleaseMutex(mutex); }
#else
    void lockserv__node_1__finite::__lock() { pthread_mutex_lock(&mutex); }
    void lockserv__node_1__finite::__unlock() { pthread_mutex_unlock(&mutex); }
#endif
...
std::ostream &operator <<(std::ostream &s, const lockserv__node_1__finite::node &t);
template <>
lockserv__node_1__finite::node _arg<lockserv__node_1__finite::node>(std::vector<ivy_value> &args, unsigned idx, long long bound);
template <>
void  __ser<lockserv__node_1__finite::node>(ivy_ser &res, const lockserv__node_1__finite::node&);
template <>
void  __deser<lockserv__node_1__finite::node>(ivy_deser &inp, lockserv__node_1__finite::node &res);
int lockserv__node_1__finite::___ivy_choose(int rng,const char *name,int id) {
        return 0;
    }
struct ivy_nondet_except {}; // lauren-yrluo added
void lockserv__node_1__finite::__init(){
        bool __tmp0[1];
        for (int N = 0; N < 1; N++) {
            __tmp0[N] = false;
        }
...
void lockserv__node_1__finite::ext__send_lock(node n){
    lock_msg[n] = true;
}
void lockserv__node_1__finite::ext__recv_lock(node n){
        ivy_assume(server_holds_lock, "lockserv.node_1.finite.ivy: line 24");
        ivy_assume(lock_msg[n], "lockserv.node_1.finite.ivy: line 25");
...
void lockserv__node_1__finite::ext__recv_grant(node n){
        ivy_assume(grant_msg[n], "lockserv.node_1.finite.ivy: line 33");
        bool __tmp5[1];
        for (int N = 0; N < 1; N++) {
...
void lockserv__node_1__finite::ext__unlock(node n){
        ivy_assume(holds_lock[n], "lockserv.node_1.finite.ivy: line 40");
        bool __tmp6[1];
        for (int N = 0; N < 1; N++) {
...
void lockserv__node_1__finite::ext__recv_unlock(node n){
        ivy_assume(unlock_msg[n], "lockserv.node_1.finite.ivy: line 47");
        bool __tmp7[1];
        for (int N = 0; N < 1; N++) {
...
bool lockserv__node_1__finite::ext__get_holds_lock(node n0){
    bool qrm_result;
    qrm_result = holds_lock[n0];
    return qrm_result;
}
bool lockserv__node_1__finite::ext__get_bool_holds_lock(node n0, bool result){
    bool qrm_result;
    qrm_result = (holds_lock[n0] == result);
    return qrm_result;
}
bool lockserv__node_1__finite::ext__get_grant_msg(node n0){
    bool qrm_result;
    qrm_result = grant_msg[n0];
    return qrm_result;
}
bool lockserv__node_1__finite::ext__get_bool_grant_msg(node n0, bool result){
    bool qrm_result;
    qrm_result = (grant_msg[n0] == result);
    return qrm_result;
}
bool lockserv__node_1__finite::ext__get_server_holds_lock(){
    bool qrm_result;
    qrm_result = server_holds_lock;
    return qrm_result;
}
bool lockserv__node_1__finite::ext__get_bool_server_holds_lock(bool result){
    bool qrm_result;
    qrm_result = (server_holds_lock == result);
    return qrm_result;
}
bool lockserv__node_1__finite::ext__get_lock_msg(node n0){
    bool qrm_result;
    qrm_result = lock_msg[n0];
    return qrm_result;
}
bool lockserv__node_1__finite::ext__get_bool_lock_msg(node n0, bool result){
    bool qrm_result;
    qrm_result = (lock_msg[n0] == result);
    return qrm_result;
}
bool lockserv__node_1__finite::ext__get_unlock_msg(node n0){
    bool qrm_result;
    qrm_result = unlock_msg[n0];
    return qrm_result;
}
bool lockserv__node_1__finite::ext__get_bool_unlock_msg(node n0, bool result){
    bool qrm_result;
    qrm_result = (unlock_msg[n0] == result);
    return qrm_result;
}
void lockserv__node_1__finite::__tick(int __timeout){
}
lockserv__node_1__finite::lockserv__node_1__finite(){
#ifdef _WIN32
...
std::ostream &operator <<(std::ostream &s, const lockserv__node_1__finite::node &t){
    if (t == lockserv__node_1__finite::node0) s<<"node0";
    return s;
}
template <>
void  __ser<lockserv__node_1__finite::node>(ivy_ser &res, const lockserv__node_1__finite::node&t){
    __ser(res,(int)t);
}
...
int ask_ret(long long bound) {
    ...
    class lockserv__node_1__finite_repl : public lockserv__node_1__finite {
    ...
    virtual void ivy_assume(bool truth,const char *msg){
        ...
        lockserv__node_1__finite_repl() : lockserv__node_1__finite(){}
...
lockserv__node_1__finite::node _arg<lockserv__node_1__finite::node>(std::vector<ivy_value> &args, unsigned idx, long long bound){
    ...
    if(arg.atom == "node0") return lockserv__node_1__finite::node0;
...
void __deser<lockserv__node_1__finite::node>(ivy_deser &inp, lockserv__node_1__finite::node &res){
    int __res;
    __deser(inp,__res);
    res = (lockserv__node_1__finite::node)__res;
}
...
class cmd_reader: public stdin_reader {
    int lineno;
public:
    lockserv__node_1__finite_repl &ivy;    

    cmd_reader(lockserv__node_1__finite_repl &_ivy) : ivy(_ivy) {
        lineno = 1;
        if (isatty(fdes()))
        ...
        try {
            ...
            ivy.__lock();
                ...
                if (action == "get_bool_grant_msg") {
                    ...
                    __ivy_out  << "= " << ivy.ext__get_bool_grant_msg(_arg<lockserv__node_1__finite::node>(args,0,1),_arg<bool>(args,1,2)) << std::endl;
                }
                else
    
                if (action == "get_bool_holds_lock") {
                    check_arity(args,2,action);
                    __ivy_out  << "= " << ivy.ext__get_bool_holds_lock(_arg<lockserv__node_1__finite::node>(args,0,1),_arg<bool>(args,1,2)) << std::endl;
                }
                else
    
                if (action == "get_bool_lock_msg") {
                    check_arity(args,2,action);
                    __ivy_out  << "= " << ivy.ext__get_bool_lock_msg(_arg<lockserv__node_1__finite::node>(args,0,1),_arg<bool>(args,1,2)) << std::endl;
                }
                else
                ...
                if (action == "get_bool_unlock_msg") {
                    check_arity(args,2,action);
                    __ivy_out  << "= " << ivy.ext__get_bool_unlock_msg(_arg<lockserv__node_1__finite::node>(args,0,1),_arg<bool>(args,1,2)) << std::endl;
                }
                else
    
                if (action == "get_grant_msg") {
                    check_arity(args,1,action);
                    __ivy_out  << "= " << ivy.ext__get_grant_msg(_arg<lockserv__node_1__finite::node>(args,0,1)) << std::endl;
                }
                else
    
                if (action == "get_holds_lock") {
                    check_arity(args,1,action);
                    __ivy_out  << "= " << ivy.ext__get_holds_lock(_arg<lockserv__node_1__finite::node>(args,0,1)) << std::endl;
                }
                else
    
                if (action == "get_lock_msg") {
                    check_arity(args,1,action);
                    __ivy_out  << "= " << ivy.ext__get_lock_msg(_arg<lockserv__node_1__finite::node>(args,0,1)) << std::endl;
                }
                else
                ...
                if (action == "get_unlock_msg") {
                    check_arity(args,1,action);
                    __ivy_out  << "= " << ivy.ext__get_unlock_msg(_arg<lockserv__node_1__finite::node>(args,0,1)) << std::endl;
                }
                else
    
                if (action == "recv_grant") {
                    check_arity(args,1,action);
                    ivy.ext__recv_grant(_arg<lockserv__node_1__finite::node>(args,0,1));
                }
                else
    
                if (action == "recv_lock") {
                    check_arity(args,1,action);
                    ivy.ext__recv_lock(_arg<lockserv__node_1__finite::node>(args,0,1));
                }
                else
    
                if (action == "recv_unlock") {
                    check_arity(args,1,action);
                    ivy.ext__recv_unlock(_arg<lockserv__node_1__finite::node>(args,0,1));
                }
                else
    
                if (action == "send_lock") {
                    check_arity(args,1,action);
                    ivy.ext__send_lock(_arg<lockserv__node_1__finite::node>(args,0,1));
                }
                else
    
                if (action == "unlock") {
                    check_arity(args,1,action);
                    ivy.ext__unlock(_arg<lockserv__node_1__finite::node>(args,0,1));
                }
                else
...
int main(int argc, char **argv){
    ...
    if (argc != 1){
        std::cerr << "usage: lockserv__node_1__finite \n";
        __ivy_exit(1);
    }
...
void ivy_exec_set_state(std::vector<std::string> state_values){
	...
	std::stringstream(state_values[2]) >> ivy_exec -> lock_msg[0];
	std::stringstream(state_values[3]) >> ivy_exec -> server_holds_lock;
