...
class PrimeOrbits():
    ...
    def symmetry_aware_enumerate(self, protocol: Protocol) -> None:
        ...
        vprint_step_banner(self.options, f'[PRIME RESULT]: Prime Orbits on [{self.options.size_str}]', 3)
    ...
    def quantifier_inference(self, atoms, tran_sys, options) -> None:
        ...
        if self.options.writeQI:
            ...
            self._write_primes(prime_filename)
        vprint_step_banner(self.options, f'[QI RESULT]: Quantified Prime Orbits on [{self.options.size_str}]', 3)
        vprint(self.options, str(self), 3)
