...
class FiniteIvyInstantiator():
    def __init__(self, tran_sys):
        ...
        self.global_vars        = [] # global non-defined + global-defined
        self.non_global_vars    = [] # local non-defined + local-defined
        self.indep_vars         = [] # non-defined
        self.initialize()

        # instantiated
        self.instantiated_global_vars       = [] 
        # self.instantiated_non_global_vars   = []
        self.instantiated_indep_vars        = []
        self.instantiated_global_equals     = [] 
        self.instantiated_non_global_equals = []
        # self.instantiated_indep_equals      = []
        self.instantiated_actions           = []
        self.instantiate()
        ...
        self.ivy_state_vars  = []
        self.ivy_actions     = []
        self.set_pretty_instantiations()

    def _init_global_vars(self):
        self.global_vars = self.tran_sys.finite_system.global_symbols
    ...
    def _get_all_params_of_param_types(self, param_types):
        ...
        for ptype in param_types:
            elems.append(self.tran_sys.sort2elems[ptype])
    ...
    def instantiate(self):
        self.instantiated_global_vars       = self._get_instantiated_terms(terms=self.global_vars,     to_equals=False)
        # self.instantiated_non_global_vars   = self._get_instantiated_terms(terms=self.non_global_vars, to_equals=False)
        self.instantiated_indep_vars        = self._get_instantiated_terms(terms=self.indep_vars,      to_equals=False)
        self.instantiated_global_equals     = self._get_instantiated_terms(terms=self.global_vars,     to_equals=True)
        self.instantiated_non_global_equals = self._get_instantiated_terms(terms=self.non_global_vars, to_equals=True)
        # self.instantiated_indep_equals      = self._get_instantiated_terms(terms=self.indep_vars,      to_equals=True)
        self.instantiated_actions           = self._get_all_parameterized_actions()
    ...
    def _set_dfs_variables(self):
        self.dfs_state_vars  = self._get_dfs_variables_from_instantiated_equals(self.instantiated_non_global_equals)
        self.dfs_global_vars = self._get_dfs_variables_from_instantiated_equals(self.instantiated_global_equals)

    def _set_protocol_atoms(self):
        atoms = self.instantiated_non_global_equals
        subst = self.tran_sys.get_pretty_set_substitution_map()
        for atom in atoms:
            self.protocol_atoms.append(printer.pretty_print_str(atom, subst).replace(' ',''))
    
    def _set_ivy_variables(self):
        ivy_vars  = self.instantiated_indep_vars
        subst = self.tran_sys.get_pretty_set_substitution_map()
        for var in ivy_vars:
            self.ivy_state_vars.append(printer.pretty_print_str(var, subst).replace(' ',''))

    def _set_ivy_actions(self):
        actions = self.instantiated_actions
        subst = self.tran_sys.get_pretty_set_substitution_map()
        for action in actions:
            self.ivy_actions.append(printer.pretty_print_str(action, subst).replace(' ',''))

    def set_pretty_instantiations(self):
