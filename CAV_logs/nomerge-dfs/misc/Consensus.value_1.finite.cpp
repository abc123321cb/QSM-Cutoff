#include "Consensus.value_1.finite.h"
...
#include <cstdint>
#endif
typedef Consensus__value_1__finite ivy_class;
...
void Consensus__value_1__finite::install_reader(reader *r) {
    #ifdef _WIN32
...
void Consensus__value_1__finite::install_thread(reader *r) {
    install_reader(r);
}

void Consensus__value_1__finite::install_timer(timer *r) {
    #ifdef _WIN32
...
#ifdef _WIN32
    void Consensus__value_1__finite::__lock() { WaitForSingleObject(mutex,INFINITE); }
    void Consensus__value_1__finite::__unlock() { ReleaseMutex(mutex); }
#else
    void Consensus__value_1__finite::__lock() { pthread_mutex_lock(&mutex); }
    void Consensus__value_1__finite::__unlock() { pthread_mutex_unlock(&mutex); }
#endif
...
std::ostream &operator <<(std::ostream &s, const Consensus__value_1__finite::value &t);
template <>
Consensus__value_1__finite::value _arg<Consensus__value_1__finite::value>(std::vector<ivy_value> &args, unsigned idx, long long bound);
template <>
void  __ser<Consensus__value_1__finite::value>(ivy_ser &res, const Consensus__value_1__finite::value&);
template <>
void  __deser<Consensus__value_1__finite::value>(ivy_deser &inp, Consensus__value_1__finite::value &res);
int Consensus__value_1__finite::___ivy_choose(int rng,const char *name,int id) {
        return 0;
    }
struct ivy_nondet_except {}; // lauren-yrluo added
void Consensus__value_1__finite::__init(){
    bool __tmp0[1];
    for (int V = 0; V < 1; V++) {
        __tmp0[V] = false;
    }
...
void Consensus__value_1__finite::ext__choose(value v){
        int __tmp1;
        __tmp1 = 1;
        for (int V = 0; V < 1; V++) {
            if (!!chosen[V]) __tmp1 = 0;
        }
        ivy_assume(__tmp1, "Consensus.value_1.finite.ivy: line 12");
        chosen[v] = true;
}
bool Consensus__value_1__finite::ext__get_chosen(value v0){
    bool qrm_result;
    qrm_result = chosen[v0];
    return qrm_result;
}
bool Consensus__value_1__finite::ext__get_bool_chosen(value v0, bool result){
    bool qrm_result;
    qrm_result = (chosen[v0] == result);
    return qrm_result;
}
void Consensus__value_1__finite::__tick(int __timeout){
}
Consensus__value_1__finite::Consensus__value_1__finite(){
#ifdef _WIN32
...
std::ostream &operator <<(std::ostream &s, const Consensus__value_1__finite::value &t){
    if (t == Consensus__value_1__finite::value0) s<<"value0";
    return s;
}
template <>
void  __ser<Consensus__value_1__finite::value>(ivy_ser &res, const Consensus__value_1__finite::value&t){
    __ser(res,(int)t);
}
...
int ask_ret(long long bound) {
    ...
    class Consensus__value_1__finite_repl : public Consensus__value_1__finite {
    ...
    virtual void ivy_assume(bool truth,const char *msg){
        ...
        Consensus__value_1__finite_repl() : Consensus__value_1__finite(){}
...
Consensus__value_1__finite::value _arg<Consensus__value_1__finite::value>(std::vector<ivy_value> &args, unsigned idx, long long bound){
    ...
    if(arg.atom == "value0") return Consensus__value_1__finite::value0;
...
void __deser<Consensus__value_1__finite::value>(ivy_deser &inp, Consensus__value_1__finite::value &res){
    int __res;
    __deser(inp,__res);
    res = (Consensus__value_1__finite::value)__res;
}
...
class cmd_reader: public stdin_reader {
    int lineno;
public:
    Consensus__value_1__finite_repl &ivy;    

    cmd_reader(Consensus__value_1__finite_repl &_ivy) : ivy(_ivy) {
        lineno = 1;
        if (isatty(fdes()))
        ...
        try {
            ...
            ivy.__lock();
                ...
                if (action == "choose") {
                    ...
                    ivy.ext__choose(_arg<Consensus__value_1__finite::value>(args,0,1));
                }
                else
    
                if (action == "get_bool_chosen") {
                    check_arity(args,2,action);
                    __ivy_out  << "= " << ivy.ext__get_bool_chosen(_arg<Consensus__value_1__finite::value>(args,0,1),_arg<bool>(args,1,2)) << std::endl;
                }
                else
    
                if (action == "get_chosen") {
                    check_arity(args,1,action);
                    __ivy_out  << "= " << ivy.ext__get_chosen(_arg<Consensus__value_1__finite::value>(args,0,1)) << std::endl;
                }
                else
...
int main(int argc, char **argv){
    ...
    if (argc != 1){
        std::cerr << "usage: Consensus__value_1__finite \n";
        __ivy_exit(1);
    }
