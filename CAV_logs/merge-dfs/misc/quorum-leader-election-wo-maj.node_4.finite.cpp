#include "quorum-leader-election-wo-maj.node_4.finite.h"
...
#include <cstdint>
#endif
typedef quorum_leader_election_wo_maj__node_4__finite ivy_class;
...
void quorum_leader_election_wo_maj__node_4__finite::install_reader(reader *r) {
    #ifdef _WIN32
...
void quorum_leader_election_wo_maj__node_4__finite::install_thread(reader *r) {
    install_reader(r);
}

void quorum_leader_election_wo_maj__node_4__finite::install_timer(timer *r) {
    #ifdef _WIN32
...
#ifdef _WIN32
    void quorum_leader_election_wo_maj__node_4__finite::__lock() { WaitForSingleObject(mutex,INFINITE); }
    void quorum_leader_election_wo_maj__node_4__finite::__unlock() { ReleaseMutex(mutex); }
#else
    void quorum_leader_election_wo_maj__node_4__finite::__lock() { pthread_mutex_lock(&mutex); }
    void quorum_leader_election_wo_maj__node_4__finite::__unlock() { pthread_mutex_unlock(&mutex); }
#endif
...
std::ostream &operator <<(std::ostream &s, const quorum_leader_election_wo_maj__node_4__finite::node &t);
template <>
quorum_leader_election_wo_maj__node_4__finite::node _arg<quorum_leader_election_wo_maj__node_4__finite::node>(std::vector<ivy_value> &args, unsigned idx, long long bound);
template <>
void  __ser<quorum_leader_election_wo_maj__node_4__finite::node>(ivy_ser &res, const quorum_leader_election_wo_maj__node_4__finite::node&);
template <>
void  __deser<quorum_leader_election_wo_maj__node_4__finite::node>(ivy_deser &inp, quorum_leader_election_wo_maj__node_4__finite::node &res);
std::ostream &operator <<(std::ostream &s, const quorum_leader_election_wo_maj__node_4__finite::nset &t);
template <>
quorum_leader_election_wo_maj__node_4__finite::nset _arg<quorum_leader_election_wo_maj__node_4__finite::nset>(std::vector<ivy_value> &args, unsigned idx, long long bound);
template <>
void  __ser<quorum_leader_election_wo_maj__node_4__finite::nset>(ivy_ser &res, const quorum_leader_election_wo_maj__node_4__finite::nset&);
template <>
void  __deser<quorum_leader_election_wo_maj__node_4__finite::nset>(ivy_deser &inp, quorum_leader_election_wo_maj__node_4__finite::nset &res);
int quorum_leader_election_wo_maj__node_4__finite::___ivy_choose(int rng,const char *name,int id) {
        return 0;
    }
struct ivy_nondet_except {}; // lauren-yrluo added
void quorum_leader_election_wo_maj__node_4__finite::__init(){
    ...
    if (qrm_solution_count == 0){
        ...
        member[3][2] = true;
        member[3][3] = true;
        quorum = nset__node0_node1_node3;
    }
    ++ qrm_solution_count;
...
void quorum_leader_election_wo_maj__node_4__finite::ext__vote(node v, node n){
        int __tmp2;
        __tmp2 = 1;
        for (int M = 0; M < 4; M++) {
            if (!!voted[v][M]) __tmp2 = 0;
        }
        ivy_assume(__tmp2, "quorum-leader-election-wo-maj.node_4.finite.ivy: line 21");
        voted[v][n] = true;
}
void quorum_leader_election_wo_maj__node_4__finite::ext__become_leader(node n, nset s){
        int __tmp3;
        __tmp3 = 1;
        for (int M = 0; M < 4; M++) {
            if (!(!member[M][s] || voted[M][n])) __tmp3 = 0;
        }
        ivy_assume(__tmp3, "quorum-leader-election-wo-maj.node_4.finite.ivy: line 26");
        isleader[n] = true;
        quorum = s;
}
bool quorum_leader_election_wo_maj__node_4__finite::ext__get_voted(node n0, node n1){
    bool qrm_result;
    qrm_result = voted[n0][n1];
    return qrm_result;
}
bool quorum_leader_election_wo_maj__node_4__finite::ext__get_bool_voted(node n0, node n1, bool result){
    bool qrm_result;
    qrm_result = (voted[n0][n1] == result);
    return qrm_result;
}
quorum_leader_election_wo_maj__node_4__finite::nset quorum_leader_election_wo_maj__node_4__finite::ext__get_quorum(){
    quorum_leader_election_wo_maj__node_4__finite::nset qrm_result;
    qrm_result = quorum;
    return qrm_result;
}
bool quorum_leader_election_wo_maj__node_4__finite::ext__get_bool_quorum(nset result){
    bool qrm_result;
    qrm_result = (quorum == result);
    return qrm_result;
}
bool quorum_leader_election_wo_maj__node_4__finite::ext__get_member(node n0, nset n1){
    bool qrm_result;
    qrm_result = member[n0][n1];
    return qrm_result;
}
bool quorum_leader_election_wo_maj__node_4__finite::ext__get_bool_member(node n0, nset n1, bool result){
    bool qrm_result;
    qrm_result = (member[n0][n1] == result);
    return qrm_result;
}
bool quorum_leader_election_wo_maj__node_4__finite::ext__get_isleader(node n0){
    bool qrm_result;
    qrm_result = isleader[n0];
    return qrm_result;
}
bool quorum_leader_election_wo_maj__node_4__finite::ext__get_bool_isleader(node n0, bool result){
    bool qrm_result;
    qrm_result = (isleader[n0] == result);
    return qrm_result;
}
void quorum_leader_election_wo_maj__node_4__finite::__tick(int __timeout){
}
quorum_leader_election_wo_maj__node_4__finite::quorum_leader_election_wo_maj__node_4__finite(){
#ifdef _WIN32
...
std::ostream &operator <<(std::ostream &s, const quorum_leader_election_wo_maj__node_4__finite::node &t){
    if (t == quorum_leader_election_wo_maj__node_4__finite::node0) s<<"node0";
    if (t == quorum_leader_election_wo_maj__node_4__finite::node1) s<<"node1";
    if (t == quorum_leader_election_wo_maj__node_4__finite::node2) s<<"node2";
    if (t == quorum_leader_election_wo_maj__node_4__finite::node3) s<<"node3";
    return s;
}
template <>
void  __ser<quorum_leader_election_wo_maj__node_4__finite::node>(ivy_ser &res, const quorum_leader_election_wo_maj__node_4__finite::node&t){
    __ser(res,(int)t);
}
std::ostream &operator <<(std::ostream &s, const quorum_leader_election_wo_maj__node_4__finite::nset &t){
    if (t == quorum_leader_election_wo_maj__node_4__finite::nset__node0_node1_node2) s<<"nset__node0_node1_node2";
    if (t == quorum_leader_election_wo_maj__node_4__finite::nset__node0_node1_node3) s<<"nset__node0_node1_node3";
    if (t == quorum_leader_election_wo_maj__node_4__finite::nset__node0_node2_node3) s<<"nset__node0_node2_node3";
    if (t == quorum_leader_election_wo_maj__node_4__finite::nset__node1_node2_node3) s<<"nset__node1_node2_node3";
    return s;
}
template <>
void  __ser<quorum_leader_election_wo_maj__node_4__finite::nset>(ivy_ser &res, const quorum_leader_election_wo_maj__node_4__finite::nset&t){
    __ser(res,(int)t);
}
...
int ask_ret(long long bound) {
    ...
    class quorum_leader_election_wo_maj__node_4__finite_repl : public quorum_leader_election_wo_maj__node_4__finite {
    ...
    virtual void ivy_assume(bool truth,const char *msg){
        ...
        quorum_leader_election_wo_maj__node_4__finite_repl() : quorum_leader_election_wo_maj__node_4__finite(){}
...
quorum_leader_election_wo_maj__node_4__finite::node _arg<quorum_leader_election_wo_maj__node_4__finite::node>(std::vector<ivy_value> &args, unsigned idx, long long bound){
    ...
    if(arg.atom == "node0") return quorum_leader_election_wo_maj__node_4__finite::node0;
    if(arg.atom == "node1") return quorum_leader_election_wo_maj__node_4__finite::node1;
    if(arg.atom == "node2") return quorum_leader_election_wo_maj__node_4__finite::node2;
    if(arg.atom == "node3") return quorum_leader_election_wo_maj__node_4__finite::node3;
...
void __deser<quorum_leader_election_wo_maj__node_4__finite::node>(ivy_deser &inp, quorum_leader_election_wo_maj__node_4__finite::node &res){
    int __res;
    __deser(inp,__res);
    res = (quorum_leader_election_wo_maj__node_4__finite::node)__res;
}
template <>
quorum_leader_election_wo_maj__node_4__finite::nset _arg<quorum_leader_election_wo_maj__node_4__finite::nset>(std::vector<ivy_value> &args, unsigned idx, long long bound){
    ivy_value &arg = args[idx];
    if (arg.atom.size() == 0 || arg.fields.size() != 0) throw out_of_bounds(idx,arg.pos);
    if(arg.atom == "nset__node0_node1_node2") return quorum_leader_election_wo_maj__node_4__finite::nset__node0_node1_node2;
    if(arg.atom == "nset__node0_node1_node3") return quorum_leader_election_wo_maj__node_4__finite::nset__node0_node1_node3;
    if(arg.atom == "nset__node0_node2_node3") return quorum_leader_election_wo_maj__node_4__finite::nset__node0_node2_node3;
    if(arg.atom == "nset__node1_node2_node3") return quorum_leader_election_wo_maj__node_4__finite::nset__node1_node2_node3;
    throw out_of_bounds("bad value: " + arg.atom,arg.pos);
}
template <>
void __deser<quorum_leader_election_wo_maj__node_4__finite::nset>(ivy_deser &inp, quorum_leader_election_wo_maj__node_4__finite::nset &res){
    int __res;
    __deser(inp,__res);
    res = (quorum_leader_election_wo_maj__node_4__finite::nset)__res;
}
...
class cmd_reader: public stdin_reader {
    int lineno;
public:
    quorum_leader_election_wo_maj__node_4__finite_repl &ivy;    

    cmd_reader(quorum_leader_election_wo_maj__node_4__finite_repl &_ivy) : ivy(_ivy) {
        lineno = 1;
        if (isatty(fdes()))
        ...
        try {
            ...
            ivy.__lock();
                ...
                if (action == "become_leader") {
                    ...
                    ivy.ext__become_leader(_arg<quorum_leader_election_wo_maj__node_4__finite::node>(args,0,4),_arg<quorum_leader_election_wo_maj__node_4__finite::nset>(args,1,4));
                }
                else
    
                if (action == "get_bool_isleader") {
                    check_arity(args,2,action);
                    __ivy_out  << "= " << ivy.ext__get_bool_isleader(_arg<quorum_leader_election_wo_maj__node_4__finite::node>(args,0,4),_arg<bool>(args,1,2)) << std::endl;
                }
                else
    
                if (action == "get_bool_member") {
                    check_arity(args,3,action);
                    __ivy_out  << "= " << ivy.ext__get_bool_member(_arg<quorum_leader_election_wo_maj__node_4__finite::node>(args,0,4),_arg<quorum_leader_election_wo_maj__node_4__finite::nset>(args,1,4),_arg<bool>(args,2,2)) << std::endl;
                }
                else
    
                if (action == "get_bool_quorum") {
                    check_arity(args,1,action);
                    __ivy_out  << "= " << ivy.ext__get_bool_quorum(_arg<quorum_leader_election_wo_maj__node_4__finite::nset>(args,0,4)) << std::endl;
                }
                else
    
                if (action == "get_bool_voted") {
                    check_arity(args,3,action);
                    __ivy_out  << "= " << ivy.ext__get_bool_voted(_arg<quorum_leader_election_wo_maj__node_4__finite::node>(args,0,4),_arg<quorum_leader_election_wo_maj__node_4__finite::node>(args,1,4),_arg<bool>(args,2,2)) << std::endl;
                }
                else
    
                if (action == "get_isleader") {
                    check_arity(args,1,action);
                    __ivy_out  << "= " << ivy.ext__get_isleader(_arg<quorum_leader_election_wo_maj__node_4__finite::node>(args,0,4)) << std::endl;
                }
                else
    
                if (action == "get_member") {
                    check_arity(args,2,action);
                    __ivy_out  << "= " << ivy.ext__get_member(_arg<quorum_leader_election_wo_maj__node_4__finite::node>(args,0,4),_arg<quorum_leader_election_wo_maj__node_4__finite::nset>(args,1,4)) << std::endl;
                }
                else
                ...
                if (action == "get_voted") {
                    check_arity(args,2,action);
                    __ivy_out  << "= " << ivy.ext__get_voted(_arg<quorum_leader_election_wo_maj__node_4__finite::node>(args,0,4),_arg<quorum_leader_election_wo_maj__node_4__finite::node>(args,1,4)) << std::endl;
                }
                else
    
                if (action == "vote") {
                    check_arity(args,2,action);
                    ivy.ext__vote(_arg<quorum_leader_election_wo_maj__node_4__finite::node>(args,0,4),_arg<quorum_leader_election_wo_maj__node_4__finite::node>(args,1,4));
                }
                else
...
int main(int argc, char **argv){
    ...
    if (argc != 1){
        std::cerr << "usage: quorum_leader_election_wo_maj__node_4__finite \n";
        __ivy_exit(1);
    }
...
#include <vector>
quorum_leader_election_wo_maj__node_4__finite_repl * ivy_exec;
cmd_reader* ivy_exec_cr;
std::ostringstream ivy_exec_stream;

void ivy_exec_init(){
	__ivy_out.basic_ios<char>::rdbuf(ivy_exec_stream.rdbuf());
	ivy_exec = new quorum_leader_election_wo_maj__node_4__finite_repl;
	ivy_exec -> __unlock();
...
void ivy_exec_set_state(std::vector<std::string> state_values){
	...
	{
		ivy_value arg; arg.pos = 0; arg.atom = state_values[4];
		std::vector<ivy_value> args; args.push_back(arg);
		ivy_exec -> quorum = _arg<quorum_leader_election_wo_maj__node_4__finite::nset>(args, 0, 0);
	}
	std::stringstream(state_values[5]) >> ivy_exec -> voted[0][0];
	std::stringstream(state_values[6]) >> ivy_exec -> voted[0][1];
	std::stringstream(state_values[7]) >> ivy_exec -> voted[0][2];
	std::stringstream(state_values[8]) >> ivy_exec -> voted[0][3];
	std::stringstream(state_values[9]) >> ivy_exec -> voted[1][0];
	std::stringstream(state_values[10]) >> ivy_exec -> voted[1][1];
	std::stringstream(state_values[11]) >> ivy_exec -> voted[1][2];
	std::stringstream(state_values[12]) >> ivy_exec -> voted[1][3];
	std::stringstream(state_values[13]) >> ivy_exec -> voted[2][0];
	std::stringstream(state_values[14]) >> ivy_exec -> voted[2][1];
	std::stringstream(state_values[15]) >> ivy_exec -> voted[2][2];
	std::stringstream(state_values[16]) >> ivy_exec -> voted[2][3];
	std::stringstream(state_values[17]) >> ivy_exec -> voted[3][0];
	std::stringstream(state_values[18]) >> ivy_exec -> voted[3][1];
	std::stringstream(state_values[19]) >> ivy_exec -> voted[3][2];
	std::stringstream(state_values[20]) >> ivy_exec -> voted[3][3];
}
