...
timeout ${Timeout} python3 qrm.py ${sym_path}Consensus.ivy                 -s value=3                      -v 5 -w -l ${sym_path}Consensus.cutoff.log
timeout ${Timeout} python3 qrm.py ${sym_path}TCommit.ivy                   -s resource_manager=2           -v 5 -w -l ${sym_path}TCommit.cutoff.log 
timeout ${Timeout} python3 qrm.py ${sym_path}Ricart-Agrawala.ivy           -s node=2                       -v 5 -w -l ${sym_path}Ricart-Agrawala.cutoff.log 
timeout ${Timeout} python3 qrm.py ${sym_path}lock_server.ivy               -s server=1,client=3            -v 5 -w -l ${sym_path}lock_server.cutoff.log                                
timeout ${Timeout} python3 qrm.py ${sym_path}sharded_kv.ivy                -s node=3,key=1,value=2         -v 5 -w -l ${sym_path}sharded_kv.cutoff.log 
